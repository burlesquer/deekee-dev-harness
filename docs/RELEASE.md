...
| `README.md` | Agent Team 제목 + Why dk-harness 테이블 | agent 수, spec 테이블 |
...
README.md                 → <h2 id="team">Agent Team (N named agents)</h2>
                          → "N agents, N skills, ..." (Why dk-harness 섹션)
...
dk-harness은 세션 시작 시 **자동으로 최신 버전을 체크**하여 status line에 `⬆ vX.Y.Z` 알림을 표시합니다.
...
**캐시:** `.dk-harness/state/version-check.json`
```json
{"latest":"2.9.0","checkedAt":1774488558688}
```

**버전 소스:** GitHub raw URL (`https://raw.githubusercontent.com/burlesquer/deekee-dev-harness/main/package.json`)
...
> npm registry(`https://registry.npmjs.org/deekee-dev-harness/latest`)로 전환하면 더 정확함.
...
```bash
# 현재 설치된 버전
cat ~/.claude/plugins/installed_plugins.json | grep -A2 '"dk-harness@dk-harness-marketplace"'
