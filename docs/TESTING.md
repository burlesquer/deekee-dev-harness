...
dk-harness uses a four-tier testing strategy with no external test framework — everything runs on
...
Scores are stored in `.dk-harness/evals/` as timestamped JSON files. Each entry records:
