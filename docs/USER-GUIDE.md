...
프로젝트 구조를 분석하고 `.dk-harness/` 런타임 디렉토리를 생성합니다.
...
```
/dk-harness do "로그인 API에 rate limit 추가해줘"
```

dk-harness이 의도를 분석해서 적절한 에이전트에게 자동 라우팅합니다.
간단한 작업은 Solo 모드 (에이전트 1명)로 빠르게 처리.

### Pattern B: "기능 하나를 처음부터 끝까지" (핵심)

```
/dk-harness auto user-auth "JWT 인증 + 리프레시 토큰"
```
...
```
/dk-harness team jay milla "결제 API 리팩토링"
```
...
│                    dk-harness 개발 플로우                       │
│                                                          │
│  1. 계획       /dk-harness plan "JWT 인증"                     │
│     ↓          Able이 요구사항 분석 + Klay이 아키텍처     │
│                                                          │
│  2. 구현       /dk-harness auto user-auth "JWT 인증"           │
│     ↓          PDCA 사이클: plan → do → check → act      │
│                                                          │
│  3. 리뷰       /dk-harness review-pipeline                     │
│     ↓          4-tier: Eng / CEO / Design / Outside      │
│                                                          │
│  4. Ship       /dk-harness ship                                │
...
결과: `.dk-harness/plans/` 에 계획 문서 저장.
...
```
/dk-harness review-pipeline          # 복잡도 기반 자동
/dk-harness review-pipeline eng      # Eng Review만
/dk-harness review-pipeline full     # 전체 4-tier
```
...
| 변경 유형 | 추천 리뷰 | 커맨드 |
|----------|----------|--------|
| 버그 수정 (1-3 파일) | Eng only | `/dk-harness review-pipeline eng` |
| API 추가 | Eng | `/dk-harness review-pipeline eng` |
| UI 변경 | Eng + Design | `/dk-harness review-pipeline` (자동) |
| 새 기능 (10+ 파일) | 전체 | `/dk-harness review-pipeline full` |
| 아키텍처 변경 | 전체 + CSO | `/dk-harness review-pipeline full` 후 `/dk-harness review cso` |
...
```
/dk-harness ship --dry-run    # 시뮬레이션 (실제 push 없음)
```
...
### "테스트 짜줘"
```
/dk-harness tdd start auth "JWT 인증 테스트"
```

### "성능이 느려"
```
/dk-harness perf runtime
```

### "이 코드 구조를 파악하고 싶어"
```
/dk-harness explore src/auth
```

### "리팩토링 하고 싶어"
```
/dk-harness refactor src/services
```
...
### "편집 범위를 제한하고 싶어"
```
/dk-harness freeze src/auth     # src/auth/ 만 편집 가능
/dk-harness unfreeze            # 해제
```
...
- `/dk-harness auto`를 적극 사용하세요. 복잡도에 따라 팀 규모가 자동 조절됩니다.
- 리뷰 후 ship하세요. Review Dashboard CLEARED가 품질 게이트입니다.
- 한국어와 영어 모두 사용 가능합니다. 자동 감지됩니다.
- `/dk-harness status`로 PDCA 진행 상황을 수시로 확인하세요.
...
- 간단한 작업: `/dk-harness do` (Solo, ~15K 토큰)
- 리뷰만: `/dk-harness review-pipeline eng` (Milla만)
- 전체 파이프라인은 복잡한 기능에만 사용

### 에러 복구

```
/dk-harness rollback         # 마지막 체크포인트로 복구
```
...
| 커맨드 | 설명 |
|--------|------|
| `/dk-harness help` | 전체 도움말 |
| `/dk-harness init` | 프로젝트 초기화 |
| `/dk-harness do <task>` | 자동 라우팅 실행 |
| `/dk-harness auto <feat> <task>` | 전체 파이프라인 |
| `/dk-harness team [agents] <task>` | 팀 직접 구성 |
| `/dk-harness wizard` | 비개발자 모드 |
| `/dk-harness plan <task>` | 계획 수립 |
| `/dk-harness explore <target>` | 코드베이스 탐색 |
| `/dk-harness debug <issue>` | 체계적 디버깅 |
| `/dk-harness review-pipeline [tier]` | 4-tier 리뷰 |
| `/dk-harness review cso` | 14-phase 보안 감사 |
| `/dk-harness ship` | 7단계 자동 Ship |
| `/dk-harness tdd start <feat> <target>` | TDD 시작 |
| `/dk-harness tdd check pass\|fail` | TDD 결과 기록 |
| `/dk-harness test` | 테스트 실행 |
| `/dk-harness qa-loop` | QA 사이클 |
| `/dk-harness perf [type]` | 성능 분석 |
| `/dk-harness refactor <scope>` | 구조적 리팩토링 |
| `/dk-harness lsp` | 죽은 코드 감지 |
| `/dk-harness task create <title>` | 태스크 생성 |
| `/dk-harness task list` | 태스크 목록 |
| `/dk-harness freeze <dir>` | 편집 범위 제한 |
| `/dk-harness unfreeze` | 제한 해제 |
| `/dk-harness retro [window]` | 엔지니어링 회고 |
| `/dk-harness rollback` | 체크포인트 복구 |
| `/dk-harness status` | 진행 상황 대시보드 |
| `/dk-harness verify-evidence` | 증거 체인 검증 |
