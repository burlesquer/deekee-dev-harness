...
dk-harness v3.0 introduces a Multi-AI Consensus Engine that coordinates three independent AI voices
...
2. **Transparency**: Every vote is logged with reasoning, visible via `dk-harness-analytics`.
3. **No silent action**: `USER_CHALLENGE` and `SECURITY_WARNING` decisions never auto-apply.
4. **Audit trail**: All consensus sessions are persisted in `.dk-harness/consensus/` as JSONL.
...
Provider availability and timeouts are configured in `.dk-harness/config.json`:
