...
const TEST_DIR = join(tmpdir(), `dk-harness-feedback-test-${Date.now()}`);
...
describe('recordFeedback', () => {
  it('JSONL 파일에 피드백을 기록한다', () => {
    ...
    const feedbackFile = join(TEST_DIR, '.dk-harness/agent-feedback/builder.json');
  ...
  it('여러 피드백을 append한다', () => {
    ...
    const content = readFileSync(join(TEST_DIR, '.dk-harness/agent-feedback/tester.json'), 'utf8');
...
describe('getAllPerformances', () => {
  it('빈 디렉토리면 빈 배열을 반환한다', () => {
    const emptyDir = join(tmpdir(), `dk-harness-empty-${Date.now()}`);
  ...
  it('모든 에이전트 성과를 반환한다', () => {
    const multiDir = join(tmpdir(), `dk-harness-multi-${Date.now()}`);
