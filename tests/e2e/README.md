...
Stores evaluation results to `~/.dk-harness/evals/` as timestamped JSON files. Supports:
...
describe('my-skill E2E', { skip: SKIP ? 'Set EVALS=1' : false }, () => {
  ...
  before(async () => {
    result = await runSession({
      prompt: '/dk-harness my-skill "task description"',
      timeout: 120_000,
    });
  });
...
Results are saved to `~/.dk-harness/evals/` with naming `{timestamp}-{testName}.json`.
