/**
 * TDD: DKH-DR Consensus Planning Quality Tests
 *
 * 기존 plan-task.test.mjs는 persist 계층만 검증.
 * 이 테스트는 DKH-DR 6자 합의 파이프라인의 구조적 완전성을 검증한다.
 *
 * 검증 대상:
 * 1. plan-manager가 DKH-DR 필수 섹션을 persist하는지 (현재 실패 예상)
 * 2. 에이전트 정의 파일에 필수 출력 형식이 있는지
 * 3. State lifecycle 전환이 SKILL.md 스펙과 일치하는지
 * 4. persist.js --stdin이 DKH-DR 확장 필드를 처리하는지
 */
...
// ────────────────────────────────────────────────────────────
// 1. plan-manager: DKH-DR 필드 지원 검증
// ────────────────────────────────────────────────────────────

describe('plan-manager: DKH-DR 필드 지원', () => {
  const TEST_DIR = join(tmpdir(), `dkhdr-plan-${Date.now()}`);
  ...
  it('should include ## Constraints section when constraints provided', async () => {
    ...
    const result = createPlan({
      ...
      goal: 'Test DKH-DR constraints persistence',
      steps: ['Step 1'],
      constraints: [
  ...
  it('should include ## Preferences section when preferences provided', async () => {
    ...
    const result = createPlan({
      ...
      goal: 'Test DKH-DR preferences persistence',
      steps: ['Step 1'],
      preferences: [
  ...
  it('should include ## Drivers section when drivers provided', async () => {
    ...
    const result = createPlan({
      feature: 'dr-drivers',
      goal: 'Test DKH-DR drivers persistence',
      steps: ['Step 1'],
      drivers: [
  ...
  it('should include ## Steelman section when steelman provided', async () => {
    ...
    const result = createPlan({
      feature: 'dr-steelman',
      goal: 'Test DKH-DR steelman persistence',
      steps: ['Step 1'],
      steelman: {
  ...
  it('should include full DKH-DR plan with all sections', async () => {
    const { createPlan } = await import('../dist/scripts/task/plan-manager.js');

    const result = createPlan({
      feature: 'full-dkhdr',
      goal: 'Complete DKH-DR plan',
    ...
    assert.equal(result.ok, true, 'Full DKH-DR plan should succeed');
    const content = readFileSync(result.planPath, 'utf-8');
    ...
    assert.equal(missingSections.length, 0,
      `Missing DKH-DR sections: ${missingSections.join(', ')}\n\nActual content:\n${content}`);
  });
});
...
describe('에이전트 정의: DKH-DR 필수 프롬프트 검증', () => {
  ...
  it('able.md should define DKH-DR output with Options + Drivers', () => {
...
describe('State Lifecycle: Phase 전환 스펙 검증', () => {
  const STATE_DIR = join(tmpdir(), `dkhdr-state-${Date.now()}`);
  const stateFile = join(STATE_DIR, '.dk-harness', 'state', 'plan-state.json');

  before(() => mkdirSync(join(STATE_DIR, '.dk-harness', 'state'), { recursive: true }));
  after(() => rmSync(STATE_DIR, { recursive: true, force: true }));

  // DKH-DR Phase 순서 (SKILL.md 기준)
...
describe('Hook Enforcement: Phase-Agent 매핑', () => {
  const HOOK_DIR = join(tmpdir(), `dkhdr-hook-${Date.now()}`);

  before(() => mkdirSync(join(HOOK_DIR, '.dk-harness', 'state'), { recursive: true }));
  ...
  it('checkAgentAllowed should permit correct agent for each phase', async () => {
    ...
    const allowed = checkAgentAllowed(HOOK_DIR, 'dk-harness:ryan');
  ...
  it('checkAgentAllowed should block wrong agent for phase', async () => {
    ...
    const blocked = checkAgentAllowed(HOOK_DIR, 'dk-harness:klay');
  ...
  it('checkAgentAllowed should not interfere when no active plan', async () => {
    const noStateDir = join(tmpdir(), `dkhdr-nostate-${Date.now()}`);
    mkdirSync(join(noStateDir, '.dk-harness', 'state'), { recursive: true });

    const { checkAgentAllowed } = await import('../dist/scripts/hooks/plan-state.js');
    const result = checkAgentAllowed(noStateDir, 'dk-harness:klay');
  ...
  it('autoAdvancePhase should advance after correct agent completes', async () => {
    const ADV_DIR = join(tmpdir(), `dkhdr-adv-${Date.now()}`);
    mkdirSync(join(ADV_DIR, '.dk-harness', 'state'), { recursive: true });
    ...
    const newPhase = autoAdvancePhase(ADV_DIR, 'dk-harness:ryan');
  ...
  it('autoAdvancePhase should NOT auto-advance from verdict phases', async () => {
    const VRD_DIR = join(tmpdir(), `dkhdr-vrd-${Date.now()}`);
    mkdirSync(join(VRD_DIR, '.dk-harness', 'state'), { recursive: true });
    ...
    const result = autoAdvancePhase(VRD_DIR, 'dk-harness:noah');
  ...
  it('full phase sequence should enforce correct agent order', async () => {
    const SEQ_DIR = join(tmpdir(), `dkhdr-seq-${Date.now()}`);
    mkdirSync(join(SEQ_DIR, '.dk-harness', 'state'), { recursive: true });
    ...
    for (const [phase, allowed, blocked] of phaseAgentPairs) {
      ...
      const allowResult = checkAgentAllowed(SEQ_DIR, `dk-harness:${allowed}`);
      assert.equal(allowResult.allowed, true, `${allowed} should be allowed in ${phase}`);

      const blockResult = checkAgentAllowed(SEQ_DIR, `dk-harness:${blocked}`);
...
// ────────────────────────────────────────────────────────────
// 5. persist.js --stdin: DKH-DR 확장 필드 처리
// ────────────────────────────────────────────────────────────

describe('persist.js --stdin: DKH-DR 확장 필드', () => {
  const STDIN_DIR = join(tmpdir(), `dkhdr-stdin-${Date.now()}`);
  ...
  it('should persist DKH-DR full payload via stdin', () => {
    const payload = JSON.stringify({
      feature: 'stdin-dkhdr',
      goal: 'Full DKH-DR via stdin',
      steps: ['Step 1', 'Step 2'],
