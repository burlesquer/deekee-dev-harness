...
const TEST_DIR = join(tmpdir(), `dk-harness-cleanup-test-${Date.now()}`);
const STATE_DIR = join(TEST_DIR, '.dk-harness', 'state');
const ARCHIVE_DIR = join(TEST_DIR, '.dk-harness', 'archive');
...
describe('session-cleanup: runSessionCleanup', () => {
  ...
  it('handles missing .dk-harness/state/ directory gracefully', async () => {
    const { runSessionCleanup } = await import('../dist/scripts/core/session-cleanup.js');
    const emptyDir = join(tmpdir(), `dk-harness-cleanup-empty-${Date.now()}`);
