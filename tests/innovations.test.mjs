/**
 * dk-harness v2.0.0 — 5 Innovations + PDCA 통합 테스트
 *
 * 1. Context Budget
 * 2. Adaptive Routing
 * 3. Evidence Chain
...
function makeTempDir() {
  const dir = join(tmpdir(), `dk-harness-test-${randomBytes(6).toString('hex')}`);
  mkdirSync(join(dir, '.dk-harness', 'state'), { recursive: true });
  return dir;
}
...
describe('Innovation #4: Self-Healing', async () => {
  ...
  describe('Recovery Engine', () => {
    ...
    it('emergency backup 존재 → 백업에서 복구', () => {
      ...
      writeFileSync(
        join(tmpDir, '.dk-harness', 'state', 'pdca-status-emergency-backup.json'),
  ...
  describe('Health Check', () => {
    ...
    it('유효한 JSON 파일 → healthy', () => {
      writeFileSync(join(tmpDir, '.dk-harness', 'state', 'pdca-status.json'), '{"version":1}');
      writeFileSync(join(tmpDir, '.dk-harness', 'project-memory.json'), '{"patterns":[]}');
    ...
    it('깨진 JSON → corrupted → unhealthy', () => {
      writeFileSync(join(tmpDir, '.dk-harness', 'state', 'pdca-status.json'), '{broken json!!!');
...
describe('Cost Ceiling (Budget Guardrail)', async () => {
  ...
  it('initCostTracker: 초기화', () => {
    ...
    const path = join(tmpDir, '.dk-harness', 'state', 'cost-tracker.json');
  ...
  it('formatCostStatus: 포맷된 상태 출력', () => {
    ...
    assert.ok(status.includes('[dk-harness Cost]'));
...
describe('Core: State Manager', async () => {
  ...
  it('writeState + readState: 원자적 쓰기/읽기', () => {
    const path = join(tmpDir, '.dk-harness', 'state', 'test.json');
  ...
  it('deleteState: 파일 삭제', () => {
    const path = join(tmpDir, '.dk-harness', 'state', 'del.json');
