...
const baseTmpDir = join('/tmp', 'dk-harness-decay-test-' + Date.now());
let testCounter = 0;
...
function setupTmpProject() {
  ...
  mkdirSync(join(tmpDir, '.dk-harness'), { recursive: true });
  writeFileSync(join(tmpDir, '.dk-harness', 'project-memory.json'), JSON.stringify({
