...
const TEST_DIR = join(tmpdir(), `dk-harness-gc-test-${Date.now()}`);
...
describe('state-gc: runGC', () => {
  ...
  it('archives removed features to .dk-harness/archive/gc-{date}.json', async () => {
    const { runGC } = await import('../dist/scripts/pdca/state-gc.js');
    writeState(STATE_PATH, {
      version: 1,
      features: {
    ...
    const archiveDir = join(TEST_DIR, '.dk-harness', 'archive');
