...
const TEST_DIR = join(tmpdir(), `dk-harness-introspect-test-${Date.now()}`);
...
describe('state-introspection: listActiveStates', () => {
  ...
  it('returns empty array for empty state dir', async () => {
    const { listActiveStates } = await import('../dist/scripts/core/state-introspection.js');
    const emptyDir = join(tmpdir(), `dk-harness-introspect-empty-${Date.now()}`);
    mkdirSync(join(emptyDir, '.dk-harness', 'state'), { recursive: true });
