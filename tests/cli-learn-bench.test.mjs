...
describe('dk-harness-bench', async () => {
  const { benchmark, runBenchSuite, formatBenchResults } = await import('../dist/scripts/cli/dk-harness-bench.js');
...
describe('dk-harness-learn', async () => {
  // Note: these tests use the real LEARNINGS_DIR (~/.dk-harness/learnings/)
  // We test with a unique slug to avoid polluting real data
  const slug = 'test-learn-' + Date.now();
  const { listLearnings, searchLearnings, addLearning, pruneLearnings, getStats } = await import('../dist/scripts/cli/dk-harness-learn.js');
  ...
  it('cleanup test data', async () => {
    ...
    const file = join(homedir(), '.dk-harness', 'learnings', `${slug}.jsonl`);
