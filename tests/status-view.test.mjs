...
const TEST_DIR = join(tmpdir(), `dk-harness-status-view-test-${Date.now()}`);
const STATE_PATH = join(TEST_DIR, '.dk-harness', 'state', 'pdca-status.json');
const STATUS_PATH = join(TEST_DIR, '.dk-harness', 'STATUS.md');
...
describe('status-view: generateStatusView', () => {
  ...
  it('creates STATUS.md at .dk-harness/STATUS.md', async () => {
  ...
  it('STATUS.md contains required header', async () => {
    ...
    assert.ok(content.includes('# dk-harness Status'), 'should have main heading');
