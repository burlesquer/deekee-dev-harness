...
function tmpDir(): string {
  const dir = fs.mkdtempSync(join(os.tmpdir(), 'dk-harness-rc-test-'));
  fs.mkdirSync(join(dir, '.dk-harness', 'logs'), { recursive: true });
  fs.mkdirSync(join(dir, '.dk-harness', 'state'), { recursive: true });
  fs.mkdirSync(join(dir, '.dk-harness', 'tasks'), { recursive: true });
  return dir;
}

function feedbackPath(dir: string): string {
  return join(dir, '.dk-harness', 'logs', 'feedback-memory.jsonl');
}
...
function writeTaskFile(dir: string, files: string[]): void {
  ...
  fs.writeFileSync(join(dir, '.dk-harness', 'tasks', 'task-test.json'), JSON.stringify(task), 'utf-8');
}
...
describe('checkAutonomyRisk — criterion (c): recurrent denial', () => {
  ...
  it('detects recurrent denial when same ruleId in recent denials', () => {
    ...
    const logPath = join(dir, '.dk-harness', 'logs', 'denials.jsonl');
...
describe('Reality check flag management', () => {
  ...
  it('readRealityCheckFlag returns null when flag.active is false', () => {
    const flagPath = join(dir, '.dk-harness', 'state', 'reality-check-flag.json');
