...
describe('loadConfig', () => {
  ...
  it('deep-merges user config over defaults', () => {
    mockStatMtime(1000);
    // First call: .dk-harness/config.json (missing), second: dk-harness.config.json (has data)
    mockReadState
...
// ---------------------------------------------------------------------------
// 3-way merge: .dk-harness/config.json > dk-harness.config.json > DEFAULTS
// ---------------------------------------------------------------------------
describe('3-way config merge', () => {
  it('.dk-harness/config.json values override dk-harness.config.json values', () => {
    mockStatMtime(1000);
    // First call: .dk-harness/config.json (profile override)
    // Second call: dk-harness.config.json (different profile override)
    mockReadState
      .mockReturnValueOnce({
        ok: true,
    ...
    // .dk-harness/config.json wins over dk-harness.config.json
    expect((config.profile as unknown as Record<string, unknown>).costMode).toBe('budget');
    // dk-harness.config.json value is preserved when .dk-harness/config.json doesn't set it
    expect((config.profile as unknown as Record<string, unknown>).maxTeamSize).toBe(3);
  });

  it('dk-harness.config.json values override DEFAULTS when .dk-harness/config.json is missing', () => {
    mockStatMtime(1000);
    mockReadState
      .mockReturnValueOnce({ ok: false, error: 'not found' })
      .mockReturnValueOnce({
