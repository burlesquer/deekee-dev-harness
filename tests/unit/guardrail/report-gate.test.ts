...
function createTaskIndex(count: number): void {
  const tasksDir = join(tmpDir, '.dk-harness', 'tasks');
...
function createReport(content: string, filename?: string): void {
  const reportsDir = join(tmpDir, '.dk-harness', 'reports');
...
describe('checkCompletionReport', () => {
  it('returns ok: true when no completed tasks (report not required)', () => {
    // No .dk-harness/tasks directory at all -> countCompletedTasks returns 0
  ...
  it('returns ok: false with templatePath when tasks completed but no report', () => {
    ...
    expect(result.templatePath).toContain('.dk-harness/reports/session-');
