 ...
 * Verifies that plans and tasks are correctly persisted to .dk-harness/ directories.
 */
...
const TEST_DIR = join(tmpdir(), `dk-harness-test-${Date.now()}`);
...
describe('Plan Manager', () => {
  ...
  it('createPlan should create .dk-harness/plans/{date}-{feature}.md', async () => {
    ...
    // Verify plan file exists in .dk-harness/plans/
    assert.ok(existsSync(result.planPath), `Plan file should exist at ${result.planPath}`);
    assert.ok(result.planPath.includes('.dk-harness/plans/'), 'Plan path should be under .dk-harness/plans/');
  ...
  it('createPlan should also create a task with subtasks', async () => {
    ...
    // Verify task file exists in .dk-harness/tasks/
    const taskPath = join(TEST_DIR, '.dk-harness', 'tasks', `${result.taskId}.json`);
...
describe('Task Manager', () => {
  ...
  it('createTask should create .dk-harness/tasks/{id}.json', async () => {
    ...
    const taskPath = join(TEST_DIR, '.dk-harness', 'tasks', `${result.taskId}.json`);
    assert.ok(existsSync(taskPath), 'Task file should exist in .dk-harness/tasks/');
  });
...
describe('persist.mjs CLI', () => {
  const CLI_TEST_DIR = join(tmpdir(), `dk-harness-cli-test-${Date.now()}`);
  ...
  it('persist.mjs plan should create files in .dk-harness/plans/', () => {
    ...
    assert.ok(result.planPath.includes('.dk-harness/plans/'), `Plan should be in .dk-harness/plans/, got: ${result.planPath}`);
    assert.ok(existsSync(result.planPath), 'Plan file should exist on disk');

    // Verify task was also created
    const tasksDir = join(CLI_TEST_DIR, '.dk-harness', 'tasks');
    assert.ok(existsSync(tasksDir), '.dk-harness/tasks/ directory should exist');
...
describe('createPlan extended fields', () => {
  const EXT_TEST_DIR = join(tmpdir(), `dk-harness-ext-test-${Date.now()}`);
...
describe('persist.mjs --stdin JSON mode', () => {
  const STDIN_TEST_DIR = join(tmpdir(), `dk-harness-stdin-test-${Date.now()}`);
  const persistPath = new URL('../dist/scripts/cli/persist.js', import.meta.url).pathname;
