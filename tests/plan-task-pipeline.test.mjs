/**
 * E2E Pipeline Test: DKH-DR plan-state 8단계 전체 흐름 검증
 *
 * 검증 대상:
...
// NOTE: .dk-harness/state 디렉토리를 미리 생성해야 acquireLock이 lock 파일을 쓸 수 있음
...
describe('checkAgentAllowed: phase별 agent 허용/차단', () => {
  ...
  it('should support dk-harness: prefix normalization', () => {
    const dir = makeDir('agent-prefix');
    try {
      initPlanState(dir, 'prefix-feature');
      advancePhase(dir, 'foundation');
      const result = checkAgentAllowed(dir, 'dk-harness:ryan');
      assert.equal(result.allowed, true);
    } finally { cleanup(dir); }
  });
...
describe('autoAdvancePhase: agent 완료 후 자동 전이', () => {
  ...
  it('should support dk-harness: prefix in completed agent name', () => {
    const dir = makeDir('auto-prefix');
    try {
      initPlanState(dir, 'auto-prefix');
      advancePhase(dir, 'foundation');
      const next = autoAdvancePhase(dir, 'dk-harness:ryan');
      assert.equal(next, 'option-design');
    } finally { cleanup(dir); }
  });
});
