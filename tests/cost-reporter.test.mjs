...
const TEST_DIR = join(tmpdir(), `dk-harness-cost-reporter-test-${Date.now()}`);
const COST_PATH = join(TEST_DIR, '.dk-harness', 'state', 'cost-tracker.json');
const TRACE_PATH = join(TEST_DIR, '.dk-harness', 'state', 'agent-traces.json');
...
describe('cost-reporter: generateCostReport', () => {
  before(() => {
    mkdirSync(join(TEST_DIR, '.dk-harness', 'state'), { recursive: true });
  });
  ...
  it('works when state files do not exist (graceful fallback)', async () => {
    const { generateCostReport } = await import('../dist/scripts/evidence/cost-reporter.js');
    const emptyDir = join(tmpdir(), `dk-harness-cost-empty-${Date.now()}`);
...
describe('cost-reporter: formatCostReport', () => {
  ...
  it('contains dk-harness Cost Report header', async () => {
    const { generateCostReport, formatCostReport } = await import('../dist/scripts/evidence/cost-reporter.js');
    writeState(COST_PATH, makeCostTracker());
    writeState(TRACE_PATH, makeTraceData());
    ...
    assert.ok(formatted.includes('dk-harness Cost Report'), 'should contain header');
  });
