/**
 * TDD: /dk-harness init 스킬 및 템플릿 시스템 단위 테스트
...
describe('init SKILL.md 구조', () => {
  ...
  it('.dk-harness/project/ 저장 경로가 명시되어야 한다', () => {
    assert.match(content, /\.dk-harness\/project\//);
  });

  it('Klay 에이전트 호출이 포함되어야 한다', () => {
    assert.match(content, /dk-harness:klay/);
  });
  ...
  it('다음 단계 안내가 있어야 한다', () => {
    assert.match(content, /\/dk-harness plan/);
    assert.match(content, /\/dk-harness do/);
  });
});
