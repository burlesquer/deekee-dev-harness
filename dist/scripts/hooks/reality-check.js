/**
 * dk-harness Reality Check — Judgment layer for feedback violation and autonomy risk detection.
...
function feedbackMemoryPath(projectDir) {
    return join(projectDir, '.dk-harness', 'logs', 'feedback-memory.jsonl');
}
function realityCheckFlagPath(projectDir) {
    return join(projectDir, '.dk-harness', 'state', 'reality-check-flag.json');
}
// ---------------------------------------------------------------------------
...
export function recordFeedback(entry, projectDir) {
    const logDir = join(projectDir, '.dk-harness', 'logs');
...
function loadAllowedFilesFromTasks(projectDir) {
    const taskDir = join(projectDir, '.dk-harness', 'tasks');
...
function hasRecentDenialForRule(ruleId, projectDir) {
    const logPath = join(projectDir, '.dk-harness', 'logs', 'denials.jsonl');
