/**
 * dk-harness Error Recovery — Tool error tracking and retry guidance.
...
function errorPath(projectDir) {
    return join(projectDir, '.dk-harness', 'state', 'error-recovery.json');
}
// ---------------------------------------------------------------------------
// Core Operations
// ---------------------------------------------------------------------------
...
export function recordToolError(projectDir, toolName, errorMsg) {
    ...
    if (entry.count >= FORCE_ALTERNATIVE) {
        const guidance = [
            `[dk-harness:error-recovery] FORCE ALTERNATIVE — ${toolName} 동일 에러 ${entry.count}회 반복`,
            `에러: ${errorMsg.slice(0, 100)}`,
            ``,
            `이 접근은 실패했습니다. 다음 중 하나를 선택하세요:`,
            `1. 완전히 다른 접근법 시도 (같은 도구/명령 사용 금지)`,
            `2. 문제를 더 작은 단위로 분해`,
            `3. /dk-harness debug 로 전환하여 근본 원인 분석`,
        ].join('\n');
    ...
    if (entry.count >= RETRY_THRESHOLD) {
        const guidance = [
            `[dk-harness:error-recovery] RETRY GUIDANCE — ${toolName} 에러 ${entry.count}회 반복`,
