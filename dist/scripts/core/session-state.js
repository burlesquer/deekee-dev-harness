/**
 * dk-harness Session State Manager
 * Provides session-scoped and stage-scoped state persistence.
 * Survives context compaction by writing to .dk-harness/state/ files.
...
function getStateDir(projectDir) {
    const dir = join(projectDir || process.cwd(), '.dk-harness', 'state');
