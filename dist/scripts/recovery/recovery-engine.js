...
export function recoverState(stateFile, projectDir) {
    ...
    if (existsSync(emergencyPath)) {
        ...
        if (result.ok && result.data?.state) {
            const restored = writeState(join(dir, '.dk-harness', 'state', stateFile), result.data.state);
            if (restored.ok) {
    ...
    const restored = writeState(join(dir, '.dk-harness', 'state', stateFile), freshState);
    if (restored.ok) {
