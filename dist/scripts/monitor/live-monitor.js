/**
 * dk-harness Live Agent Monitor v1.0.0
 * Watches .dk-harness/state/agent-live.jsonl for real-time agent activity.
...
function printHeader() {
    console.log(`\n${BOLD}━━━ dk-harness live monitor ━━━${RESET}`);
...
export function writeLiveEvent(projectDir, event) {
    const dir = join(projectDir, '.dk-harness', 'state');
...
const liveFile = join(projectDir, '.dk-harness', 'state', 'agent-live.jsonl');
...
// Default: watch mode
mkdirSync(join(projectDir, '.dk-harness', 'state'), { recursive: true });
startWatch(liveFile);
// Keep process alive
setInterval(() => { }, 1000);
// Clean exit
