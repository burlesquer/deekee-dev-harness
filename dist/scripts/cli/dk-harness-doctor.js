/**
 * dk-harness Doctor — Installation health check
 * @module scripts/cli/dk-harness-doctor
 */
...
export function runHealthCheck(projectDir) {
    ...
    // .dk-harness runtime
    checks.push({
        name: 'Runtime (.dk-harness/)',
        status: existsSync(join(dir, '.dk-harness')) ? 'OK' : 'NOT INITIALIZED',
        detail: 'Run /dk-harness init to initialize',
    });
    // Node.js version
...
export function formatHealthCheck(result) {
    const lines = [`dk-harness Doctor: ${result.healthy ? 'HEALTHY' : 'ISSUES FOUND'}`, ''];
...
//# sourceMappingURL=dk-harness-doctor.js.map