...
function runE2ETier(skill, projectDir) {
    ...
    const maxScore = 100;
    try {
        const testPrompt = `Run /dk-harness ${skill} --dry-run and report if it initializes without errors. Reply ONLY with JSON: {"success": true/false, "error": "..."}`;
...
export function runEvalSuite(tiers, projectDir, skillFilter) {
    ...
    const stateDir = join(dir, '.dk-harness', 'evals');
...
export function loadLatestEval(projectDir) {
    const dir = projectDir || process.cwd();
    const evalsDir = join(dir, '.dk-harness', 'evals');
