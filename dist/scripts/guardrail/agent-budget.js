/**
 * dk-harness Agent Budget Guard v1.0.0
...
function budgetPath(projectDir) {
    return join(projectDir, '.dk-harness', 'state', 'agent-budget.json');
}
...
/**
 * Track an agent spawn. Called by pre-tool-use hook on every dk-harness: agent spawn.
...
export function trackAgentSpawn(projectDir, agentName) {
    ...
    if (state.agentCount > MAX_SESSION_AGENTS) {
        result.warn = `[dk-harness:agent-budget] Session agent limit (${MAX_SESSION_AGENTS}) exceeded (current: ${state.agentCount}). Consider finalizing current work.`;
        ...
        result.warn = `[dk-harness:agent-budget] Approaching agent limit — this is agent ${state.agentCount}/${MAX_SESSION_AGENTS}.`;
    }
    if (state.totalDurationMs > MAX_TOTAL_AGENT_DURATION_MS) {
        result.warn = `[dk-harness:agent-budget] Total agent time (${Math.round(state.totalDurationMs / 60000)}min) exceeds budget (${MAX_TOTAL_AGENT_DURATION_MS / 60000}min). Consider finalizing.`;
    }
...
export function recordAgentCompletion(projectDir, agentName, durationMs) {
    ...
    if (durationMs > SLOW_AGENT_WARN_MS) {
        ...
        warn = `[dk-harness:slow-agent] ${agentName} took ${Math.round(durationMs / 1000)}s (>${SLOW_AGENT_WARN_MS / 1000}s). Consider using sonnet model or reducing prompt complexity.`;
