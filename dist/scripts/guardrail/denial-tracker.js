/**
 * dk-harness Denial Tracker v1.0.0
...
export function recordDenial(entry, projectDir) {
    ...
    try {
        const logDir = join(dir, '.dk-harness', 'logs');
    ...
    try {
        const statePath = join(dir, '.dk-harness', 'state', 'denial-audit.json');
...
export function getDenialSummary(projectDir) {
    ...
    if (state.sessionDenials === 0)
        return null;
    const lines = [
        `[dk-harness:denial-audit] ${state.sessionDenials} guardrail denial(s) this session:`,
    ];
    ...
    lines.push(`  Log: .dk-harness/logs/denials.jsonl`);
    return lines.join('\n');
}
/**
