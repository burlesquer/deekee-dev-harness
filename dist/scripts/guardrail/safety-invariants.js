/**
 * dk-harness Safety Invariants v0.3.0
...
function getInvariantsPath(projectDir) {
    return join(projectDir || process.cwd(), '.dk-harness', 'state', 'invariants-tracker.json');
}
/**
...
export function checkStepLimit(projectDir) {
    ...
    if (steps > invariants.maxSteps) {
        ...
        return {
            ...
            message: `[dk-harness Safety] 실행 단계 한도 초과 (${steps}/${invariants.maxSteps}). 작업을 분할하거나 설정에서 maxSteps를 조정하세요.`
        };
    }
...
export function checkFileChangeLimit(filePath, projectDir) {
    ...
    if (fileChanges > invariants.maxFileChanges) {
        ...
        return {
            ...
            message: `[dk-harness Safety] 파일 변경 한도 초과 (${fileChanges}/${invariants.maxFileChanges}). 커밋 후 계속하세요.`
        };
    }
...
export function checkForbiddenPath(filePath, projectDir) {
    ...
    for (const forbidden of invariants.forbiddenPaths) {
        ...
        if (expanded.startsWith(expandedForbidden)) {
            ...
            return {
                ok: false,
                message: `[dk-harness Safety] 🚫 접근 금지 경로: ${filePath}`
            };
        }
    }
    return { ok: true };
...
export function checkErrorLimit(projectDir) {
    ...
    if (errors > invariants.maxConsecutiveErrors) {
        return {
            ...
            message: `[dk-harness Safety] 연속 에러 한도 초과 (${errors}/${invariants.maxConsecutiveErrors}). 접근 방식을 변경하거나 도움을 요청하세요.`
        };
    }
