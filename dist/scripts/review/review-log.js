/**
 * dk-harness Review Log — JSONL persistence for review results.
...
export function appendReviewLog(entry, projectDir) {
    ...
    const logDir = join(dir, '.dk-harness', 'reviews');
...
export function readReviewLog(projectDir) {
    const dir = projectDir || process.cwd();
    const logPath = join(dir, '.dk-harness', 'reviews', 'review-log.jsonl');
