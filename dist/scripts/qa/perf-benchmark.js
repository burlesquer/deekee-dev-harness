/**
 * dk-harness Performance Benchmark — Regression Detection via Threshold Comparison
...
/**
 * Load performance baseline from .dk-harness/qa/perf-baseline.json.
 */
export function loadBaseline(projectDir) {
    const path = join(projectDir, '.dk-harness', 'qa', 'perf-baseline.json');
    const data = readStateOrDefault(path, {});
    return data;
}
/**
 * Save performance metrics as new baseline to .dk-harness/qa/perf-baseline.json.
 */
export function saveBaseline(projectDir, metrics) {
    const path = join(projectDir, '.dk-harness', 'qa', 'perf-baseline.json');
    mkdirSync(join(projectDir, '.dk-harness', 'qa'), { recursive: true });
