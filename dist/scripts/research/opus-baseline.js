 ...
 * Output: .dk-harness/research/opus-cost-ranking.md
 *
...
import { MODEL_REGISTRY } from '../routing/model-router.js';
...
export function tierForAgent(normalized) {
    return AGENT_DEFAULTS[normalized] ?? 'sonnet';
}
export function parseReplayLines(lines) {
    const out = [];
    for (const line of lines) {
        ...
        if (obj?.event !== 'agent_stop')
            continue;
        if (typeof obj.agent_type !== 'string' || typeof obj.duration_ms !== 'number')
            continue;
        out.push({ agent_type: obj.agent_type, duration_ms: obj.duration_ms });
    }
    return out;
}
export function aggregateByAgent(events) {
    const map = new Map();
...
export function formatRankingMarkdown(map, meta) {
    const sorted = [...map.values()].sort((a, b) => b.costProxy - a.costProxy);
    const top5 = sorted.slice(0, 5);
    const row = (r, i) => `| ${i + 1} | ${r.agent} | ${r.tier} | ${r.calls} | ${r.totalDurationMs} | ${r.costProxy.toLocaleString()} |`;
...
// ---------------- CLI entry (read-only) ----------------
function loadReplayFiles(stateDir) {
...
function loadSubagentTracking(stateDir) {
    ...
    try {
        ...
        return agents
            .filter((a) => typeof a?.agent_type === 'string' && typeof a?.duration_ms === 'number')
            .map((a) => ({ agent_type: a.agent_type, duration_ms: a.duration_ms }));
    }
    catch {
        return [];
    }
...
//# sourceMappingURL=opus-baseline.js.map