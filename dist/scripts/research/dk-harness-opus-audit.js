#!/usr/bin/env node
/**
 * dk-harness Opus Audit — 4-way Decision Map (Phase A, read-only static analyzer)
 *
 * Maps each dk-harness agent across 5 model decision sources and classifies it:
 ...
 *
 * Output: .dk-harness/research/dk-harness-opus-audit.md
 *
 * @module scripts/research/dk-harness-opus-audit
 */
import { readFileSync, writeFileSync, mkdirSync, readdirSync, existsSync } from 'node:fs';
import { join, dirname, basename } from 'node:path';
import { fileURLToPath } from 'node:url';
// ---------- Pure Parsers ----------
export function parseFrontmatterModel(content) {
...
function extractObjectBlock(source, declRegex) {
...
export function parseAgentDefaults(source) {
...
function upgrade(prev, next) {
    if (!prev)
        return next;
    return RANK[next] > RANK[prev] ? next : prev;
}
export function parseAgentTiers(source) {
    ...
    while ((m = re.exec(block)) !== null) {
        ...
        out[agent] = upgrade(out[agent], m[2]);
    }
    return out;
}
export function parseOrchestratorHardcoded(source) {
    const out = {};
...
export function parseRosterFromClaudeMd(content) {
...
// ---------- Classifier ----------
export function classifyAgent(s) {
    if (s.frontmatter !== 'opus')
        return 'JUSTIFIED';
    const others = [s.agentTiers, s.orchestrator, s.roster];
    const otherOpus = others.filter((v) => v === 'opus').length;
    if (otherOpus === others.length)
        return 'JUSTIFIED';
    if (otherOpus === 0)
        return 'ORPHAN_OPUS';
    return 'DIVERGENT';
...
export function formatAuditMarkdown(report) {
    ...
    const lines = [];
    lines.push('# dk-harness Opus Audit — 4-way Decision Map');
    lines.push('');
    ...
    const sepActive = '|---|---|---|---|---|';
    const rowActive = (a) => `| ${a.name} | ${fmt(a.sources.frontmatter)} | ${fmt(a.sources.agentTiers)} | ${fmt(a.sources.orchestrator)} | ${fmt(a.sources.roster)} |`;
    lines.push('## 🔴 ORPHAN_OPUS (다운그레이드 후보)');
    if (groups.ORPHAN_OPUS.length === 0)
        lines.push('_없음_');
    else {
        lines.push(headerActive);
        lines.push(sepActive);
        for (const a of groups.ORPHAN_OPUS)
            lines.push(rowActive(a));
    }
    lines.push('');
    ...
    if (groups.DIVERGENT.length === 0)
        ...
        lines.push(headerActive);
        lines.push(sepActive);
        for (const a of groups.DIVERGENT)
            lines.push(rowActive(a));
    }
    lines.push('');
    lines.push('## 🟢 JUSTIFIED');
    if (groups.JUSTIFIED.length === 0)
        lines.push('_없음_');
    else {
        lines.push(headerActive);
        lines.push(sepActive);
        for (const a of groups.JUSTIFIED)
            lines.push(rowActive(a));
    }
    lines.push('');
    lines.push('## 전체 매핑 표');
    ...
    for (const a of report.agents) {
        lines.push(`| ${a.name} | ${fmt(a.sources.frontmatter)} | ${fmt(a.sources.agentDefaults)} | ${fmt(a.sources.agentTiers)} | ${fmt(a.sources.orchestrator)} | ${fmt(a.sources.roster)} | ${a.classification} |`);
    }
    lines.push('');
...
export function main(repoRoot) {
    ...
    const outDir = join(repoRoot, '.dk-harness/research');
    if (!existsSync(outDir))
        mkdirSync(outDir, { recursive: true });
    const outPath = join(outDir, 'dk-harness-opus-audit.md');
...
//# sourceMappingURL=dk-harness-opus-audit.js.map