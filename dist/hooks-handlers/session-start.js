...
try {
    ...
    const pdcaState = readStateOrDefault(join(projectDir, '.dk-harness', 'state', 'pdca-status.json'), null);
    const memory = readStateOrDefault(join(projectDir, '.dk-harness', 'project-memory.json'), null);
    const ctx = [];
    // === Header ===
    ...
    const injectionMode = process.env.DK_FULL_INJECT === '1' ? 'full' : (config.context?.injectionMode || 'compact');
    ...
    if (!setupStatus.completed) {
        const hudSetupFlag = join(projectDir, '.dk-harness', 'state', 'hud-setup-done');
        if (!existsSync(hudSetupFlag)) {
            try {
                const stateDir = join(projectDir, '.dk-harness', 'state');
