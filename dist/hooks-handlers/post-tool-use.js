...
try {
    ...
    if (toolName) {
        ...
        if ((toolName === 'Agent' || toolName === 'Task') && toolInput.subagent_type) {
            const agentKey = toolInput.name || toolInput.subagent_type.replace('dk-harness:', '');
            const agentName = toolInput.name || toolInput.subagent_type.replace('dk-harness:', '');
            ...
            const agentKey = toolInput.to.replace('dk-harness:', '');
    ...
    if (toolName === 'Bash' && toolResponse) {
        ...
        if (ev)
            process.stderr.write(`[dk-harness:evidence] ${ev.type}: ${ev.result}\n`);
    ...
    process.stderr.write(`[dk-harness:post-tool-use] ${err.message}\n`);
    process.stdout.write('{}');
}
