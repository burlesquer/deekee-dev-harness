...
describe('checkFilePath', () => {
    ...
    it('violation messages include [dk-harness guardrail] tag', () => {
        const result = checkFilePath('.env');
        const envViolation = result.violations.find(v => v.rule.id === 'protect-env');
        expect(envViolation?.rule.message).toContain('[dk-harness guardrail]');
    });
...
describe('formatViolations', () => {
    ...
    it('formats block violations with header', () => {
        ...
        expect(output).toContain('[dk-harness Guardrail]');
