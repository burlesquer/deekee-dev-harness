...
# /dk-harness:task — Task Checklist Manager

## Actions

| 명령 | 설명 |
|------|------|
| `/dk-harness:task create <title> --subtasks "s1, s2, s3"` | 태스크 생성 |
| `/dk-harness:task from-plan [plan-path]` | 플랜의 Steps를 태스크로 변환 |
| `/dk-harness:task check <task-id> <seq>` | 서브태스크 완료 체크 ☐→☑ |
| `/dk-harness:task list` | 전체 태스크 목록 |
| `/dk-harness:task show <task-id>` | 상세 체크리스트 |
| `/dk-harness:task add <task-id> "subtask title"` | 서브태스크 추가 |
...
task-cli.js가 없으면 오케스트레이터가 직접 `.dk-harness/tasks/task-{id}.json`에 Write:
...
1. `plan-path`가 없으면 `.dk-harness/plans/`에서 가장 최근 플랜 파일을 Read
...
1. `.dk-harness/tasks/{task-id}.json`을 Read
...
`.dk-harness/tasks/`의 모든 JSON 파일을 읽어 테이블로 표시:
...
- 태스크 파일: `.dk-harness/tasks/task-{id}.json`
- 태스크 인덱스: `.dk-harness/tasks/index.json` (선택 — list 성능 최적화)
