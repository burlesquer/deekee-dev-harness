...
# /dk-harness progress-check — Implementation Progress Analysis

기획 문서와 프로젝트 코드베이스를 비교하여 구현 진행도를 산출합니다.

## Usage
```
/dk-harness progress-check
/dk-harness progress-check --spec .dk-harness/designs/figma-spec.md --project /path/to/project
/dk-harness progress-check --spec docs/PRD.md
```
...
1. `--spec` 미제공 → 기본값 `.dk-harness/designs/figma-spec.md` 사용, 없으면 "기획 문서 경로를 알려주세요"
...
```
Agent({
  subagent_type: "dk-harness:progress-checker",
...
`.dk-harness/reports/progress-report.md`에 저장.

## Output
- `.dk-harness/reports/progress-report.md` — 진행도 리포트
- 콘솔에 요약 테이블 출력
