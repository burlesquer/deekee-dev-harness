...
description: |
  ...
  /dk-harness ship creates the PR. Use when asked to "land", "deploy",
  "merge and deploy", or "ship to production".
---

# /dk-harness land-and-deploy — Full Deployment Pipeline
...
```
/dk-harness ship (PR 생성)
    ↓
/dk-harness land-and-deploy
...
## Prerequisites
- PR must exist (from `/dk-harness ship`)
...
```
/dk-harness land-and-deploy PR#123 feature-name
/dk-harness land-and-deploy PR#123 feature-name --canary-url https://app.example.com
/dk-harness land-and-deploy PR#123 feature-name --dry-run
```
...
## Integration with /dk-harness ship

Full chain: `/dk-harness ship` → PR created → (review + approval) → `/dk-harness land-and-deploy` → production
