...
# /dk-harness ai-pipeline — AI 자동 개발 파이프라인
...
```
/dk-harness ai-pipeline "<태스크 설명>"
/dk-harness ai-pipeline "이미지 분류 모델을 브라우저에서 실행"
/dk-harness ai-pipeline "한국어 감성 분석 API 만들어줘"
/dk-harness ai-pipeline "음성 인식 모바일 앱에 넣어줘"
```
...
→ .dk-harness/ai-pipeline/candidates.md"
)
```
...
Read: .dk-harness/ai-pipeline/candidates.md
...
→ .dk-harness/ai-pipeline/comparison.md
→ .dk-harness/ai-pipeline/selected-model.md (Jo용 Implementation Spec)"
)
```
...
```
Agent(
  subagent_type="dk-harness:jo",
  name="jo",
  prompt="[AI PIPELINE — Phase 3: IMPLEMENT]

Read: .dk-harness/ai-pipeline/selected-model.md
...
→ .dk-harness/ai-pipeline/test-report.md"
)
```
...
각 Phase의 상태를 `.dk-harness/ai-pipeline/` 디렉토리에 저장:

```
.dk-harness/ai-pipeline/
