...
```
/dk-harness harness "코드 리뷰 팀 만들어"     ← 기본: 자동 설계 (find→설계→sim→check 통합)
/dk-harness harness check                     ← 기존 하네스 검증
/dk-harness harness find "리서치"             ← 패턴 검색/추천
/dk-harness harness sim                       ← 데이터 흐름 시뮬레이션
/dk-harness harness log                       ← 버전 이력 + A/B 비교
/dk-harness harness chain "a → b → c"        ← 멀티 하네스 파이프라인
/dk-harness harness fix                       ← 런타임 진단
```
