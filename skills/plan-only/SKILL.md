...
# /dk-harness:plan-only — 에이전트 없는 경량 플래닝

에이전트 호출 0회. 오케스트레이터(당신)가 직접 플랜을 작성하고 persist한다.
plan-task의 DKH-DR 합의 파이프라인이 과잉일 때 사용.
...
```
/dk-harness:plan-only <task-description>
/dk-harness:plan-only "사용자 프로필 API 추가"
/dk-harness:plan-only "로그인 버그 수정"
```
...
AskUserQuestion:
1. **/dk-harness:auto** — 바로 실행
2. **/dk-harness:team** — 팀 실행 (verify→fix 루프)
3. **/dk-harness:task** — Steps를 체크리스트로 변환 (수동 추적용)
4. **저장만** — 나중에 실행

### Option 3 선택 시: Task 체크리스트 생성

플랜의 Steps를 `/dk-harness:task create`로 변환한다:

```bash
# 플랜의 Steps를 쉼표 구분 서브태스크로 변환
/dk-harness:task create "{feature}" --subtasks "{step1}, {step2}, {step3}"
```

생성 후 체크리스트를 표시하고 종료. 사용자가 나중에 `/dk-harness:task check`로 진행 추적.
