...
터미널 표시:
```
⏺ dk-harness:milla(Milla: 코드 품질 + 보안 점검) Sonnet
⏺ dk-harness:sam(Sam: 증거 체인 검증 + 최종 판정) Haiku
```
...
1. Run `/dk-harness qa` with the project test command
...
| 이전과 동일 REJECT 사유 | REJECTED + escalate | → team-fix + `/dk-harness debug` 제안 |
