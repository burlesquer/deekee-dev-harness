...
- **7회 반복**: `/dk-harness debug` 자동 전환 제안
...
1. Read `.dk-harness/handoffs/{feature}/team-verify-*.md` for verification findings
2. Read `.dk-harness/handoffs/{feature}/team-architect-*.md` for architect feedback (있으면)
...
- `/dk-harness debug` 로 전환하여 근본 원인 분석 제안
...
터미널 표시:
```
⏺ dk-harness:jay(Jay: Fix #1 — lint 에러 수정) Sonnet
```
