...
TeamCreate({
  ...
  description: "dk-harness team: <task>"
})
```
...
team-verify uses the same complexity-based review scaling as `/dk-harness review`:
- low: Milla only
- mid: Milla + Klay
...
```
Agent({
  subagent_type: "dk-harness:{original_agent}",
