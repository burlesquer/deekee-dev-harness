...
**Skip 조건**: `--plan <path>` 제공 시 또는 `/dk-harness plan`에서 전환된 경우 이 단계를 건너뜁니다.
...
| **high** | ≥ 7 | `/dk-harness plan-task` 리다이렉트 | 6자 합의 필요 → plan 완료 후 team-exec으로 복귀 |
...
```
Skill("dk-harness:plan-task") 호출 → plan 완료 후 team-exec으로 복귀
...
**persist 실패 시 fallback**: CLI arg 모드로 재시도. 재실패 시 plan을 `.dk-harness/plans/` 에 직접 Write로 저장.

## 출력 아티팩트
- Plan file: `.dk-harness/plans/{date}-{feature}.md` ← persist.js가 생성
- Task file: `.dk-harness/tasks/task-{id}.json` ← persist.js가 생성
