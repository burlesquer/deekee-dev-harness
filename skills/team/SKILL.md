...
# /dk-harness team — Staged Team Pipeline
...
```
/dk-harness team jay+derek+milla "사용자 인증 API 구현"
/dk-harness team "검색 기능 추가"              ← 에이전트 미지정 시 자동 선택
/dk-harness team --plan .dk-harness/plans/xxx.md   ← 기존 plan 파일로 team-plan 스킵
```

## Step 0: Resume Detection

1. Check `.dk-harness/state/team-session.json` for an active session
...
```
/dk-harness team jay+derek "task"
→ Execution: Jay, Derek
...
- **high (≥7)**: `/dk-harness plan-task`로 리다이렉트 (6자 합의 후 team-exec으로 복귀)
...
- 동일 에러 지속 시 `/dk-harness debug` 자동 제안
