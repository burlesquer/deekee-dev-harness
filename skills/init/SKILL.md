...
# /dk-harness init — Project Initialization

GSD의 new-project 방식을 dk-harness 스타일로 재해석한 프로젝트 초기화 스킬입니다.
질문 기반 문맥 수집 후 `.dk-harness/project/` 에 프로젝트 문맥 파일을 생성합니다.
...
- `--reset` → 기존 `.dk-harness/project/` 삭제 후 재초기화
...
USAGE:
  /dk-harness init                   인터랙티브 프로젝트 초기화
  /dk-harness init <project-name>    프로젝트명 지정 후 초기화
  /dk-harness init --detect          코드베이스 자동 감지만 수행
  /dk-harness init --reset           기존 초기화 데이터 삭제 후 재실행
  /dk-harness init --help            이 도움말

OUTPUT:
  .dk-harness/project/PROJECT.md      프로젝트 정의
  .dk-harness/project/REQUIREMENTS.md 요구사항 체크리스트
  .dk-harness/project/TECH-STACK.md   기술 스택 분석

NEXT STEPS:
  /dk-harness plan <feature>   기능 계획 수립
  /dk-harness do <task>        작업 실행
  /dk-harness tdd start        TDD 사이클 시작
```

## Pre-Init Check: Already Initialized?

`.dk-harness/project/PROJECT.md` 가 존재하는지 확인:

```bash
test -f .dk-harness/project/PROJECT.md && echo "exists" || echo "fresh"
```
...
**Option 1 선택 시:** `.dk-harness/project/PROJECT.md` 읽어서 출력 후 종료.
**Option 2 선택 시:** Phase 2 (Q1~Q3 스킵, Q4~Q5만) → Phase 3 (REQUIREMENTS.md만 재생성).
**Option 3 선택 시:** `rm -rf .dk-harness/project/` 후 전체 Phase 실행.
**Option 4 선택 시:** 종료.

### --reset 플래그

```bash
rm -rf .dk-harness/project/
```
...
`templates/project.md` 템플릿을 참조하여 `.dk-harness/project/PROJECT.md` 생성.
...
`templates/requirements.md` 템플릿을 참조하여 `.dk-harness/project/REQUIREMENTS.md` 생성.
...
`templates/tech-stack.md` 템플릿을 참조하여 `.dk-harness/project/TECH-STACK.md` 생성.
...
- 기존 `.dk-harness/project/` 외부 파일은 절대 수정하지 않음
