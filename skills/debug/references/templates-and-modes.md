...
```bash
grep -rl "Status: OPEN\|Status: INVESTIGATING" .dk-harness/debug/ 2>/dev/null | sort
```

결과가 없으면: "활성 디버그 세션이 없습니다. `/dk-harness debug <증상>` 으로 새 세션을 시작하세요."
...
선택된 `.dk-harness/debug/{date}-{slug}.md` 를 읽고:
- 마지막 미테스트 가설부터 계속
...
```bash
ls .dk-harness/debug/*.md 2>/dev/null
```
