...
# /dk-harness design — UI Design Generation & Editing

## Usage
```
/dk-harness design <prompt>
/dk-harness design generate "대시보드 랜딩페이지"
/dk-harness design enhance "로그인 페이지 만들어줘"
/dk-harness design edit "헤더에 검색바 추가"
```
...
```
Agent({
  subagent_type: "dk-harness:willji",
...
1. **Context 확인**: `.dk-harness/designs/DESIGN.md` 존재 여부 체크
...
4. **Asset 저장**: `.dk-harness/designs/{page}.html`, `.dk-harness/designs/{page}.png`
...
2. `.dk-harness/designs/DESIGN.md` 존재 시 디자인 시스템 컨텍스트 주입
...
1. 기존 화면 ID 확인 (`.dk-harness/designs/metadata.json`)
...
모든 디자인 산출물은 `.dk-harness/designs/`에 저장:
```
.dk-harness/designs/
