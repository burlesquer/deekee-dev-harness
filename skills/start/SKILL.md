---
name: start
description: "dk-harness 초기 셋업 위자드. 스코프 선택, HUD 설정, 실행 모드 구성을 인터랙티브로 진행."
triggers: ["start", "setup", "초기설정", "셋업", "setup dk-harness", "dk-harness setup"]
metadata:
  filePattern: []
  bashPattern: []
---

# /dk-harness start — Interactive Setup Wizard

dk-harness 첫 실행 또는 재설정 시 인터랙티브 셋업을 진행합니다.
...
```
dk-harness Setup - Harness Engineering Agent 초기 설정

USAGE:
  /dk-harness start              초기 셋업 위자드 (또는 이미 설정된 경우 업데이트)
  /dk-harness start --local      이 프로젝트만 설정 (.claude/CLAUDE.md)
  /dk-harness start --global     전체 프로젝트 설정 (~/.claude/CLAUDE.md)
  /dk-harness start --force      전체 위자드 강제 재실행
  /dk-harness start --help       이 도움말
```
...
AskUserQuestion: "dk-harness이 이미 설정되어 있습니다."
