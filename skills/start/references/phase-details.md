...
**Question:** "dk-harness을 어디에 설정하시겠습니까?"

**Options:**
1. **이 프로젝트만 (Local)** — `.claude/CLAUDE.md`에 설정. 이 프로젝트에서만 dk-harness 활성화.
2. **모든 프로젝트 (Global)** — `~/.claude/CLAUDE.md`에 설정. 전체 Claude Code 세션에서 dk-harness 활성화.
...
3. `<!-- DK:START -->` ~ `<!-- DK:END -->` 마커가 이미 있으면 그 사이만 교체
4. 마커가 없으면 파일 끝에 추가

**삽입할 내용:**
```markdown
<!-- DK:START -->
# dk-harness Harness Engineering Agent v2.0.0

dk-harness이 활성화되어 있습니다. 세션 시작 시 자동으로 harness rules가 주입됩니다.
...
## Quick Start
- `/dk-harness auto <feature> <task>` — 전체 파이프라인 자동 실행
- `/dk-harness start <name>` — PDCA 사이클 시작
- `/dk-harness wizard` — 비개발자 마술사 모드
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node $HOME/.claude/hud/dk-harness-hud.mjs"
  }
}
```
**IMPORTANT:** settings.json의 다른 설정은 보존합니다. `statusLine` 키만 추가/교체합니다.

2. `~/.claude/hud/dk-harness-hud.mjs` 래퍼가 없으면 생성:
```javascript
#!/usr/bin/env node
...
const candidates = [
  ...
  join(home, '.claude/plugins/cache/dk-harness-marketplace/dk-harness'),
].filter(Boolean);
...
try {
  ...
  process.stdout.write('dk-harness');
}
```
...
### "아니오" 선택 시:
`HUD_ENABLED=false` 기록. 나중에 `/dk-harness start --force`로 재설정 가능.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  * Sam(CTO)    @ Able(PM)      ^ Klay(Arch)
  ...
  시작하려면:
  ─────────────────────────────────────
  /dk-harness auto <feature> <task>  전체 자동 파이프라인
  /dk-harness start <name>           PDCA 사이클 시작
  /dk-harness wizard                 마술사 모드
  /dk-harness help                   전체 도움말
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
gh api user/starred/burlesquer/deekee-dev-harness &>/dev/null
```

아직 star 안 했으면 AskUserQuestion → "네!" → `gh api -X PUT /user/starred/burlesquer/deekee-dev-harness`
...
1. `.dk-harness/state/setup-complete.json` 또는 `~/.claude/.dk-harness-config.json` 확인
2. **설정 안 됨:** "`/dk-harness start`를 실행하세요" 메시지 추가
