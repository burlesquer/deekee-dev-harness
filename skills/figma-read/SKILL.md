...
# /dk-harness figma-read — Figma Spec Extraction

Figma 파일을 읽어 구조화된 기획 문서를 생성합니다.

## Usage
```
/dk-harness figma-read <figma-url>
/dk-harness figma-read "https://figma.com/design/abc123/MyProject?node-id=1-2"
```
...
```
Agent({
  subagent_type: "dk-harness:figma-reader",
...
터미널 표시:
```
⏺ dk-harness:figma-reader(Figma Reader: 기획 문서 추출) Opus
```
...
추출한 데이터를 5개 섹션으로 구조화하여 `.dk-harness/designs/figma-spec.md`에 저장.
...
```
/dk-harness progress-check --spec .dk-harness/designs/figma-spec.md
```

→ Progress Checker 에이전트가 "분석할 프로젝트 경로를 알려주세요"라고 질문합니다.

## Output
- `.dk-harness/designs/figma-spec.md` — 구조화된 기획 문서
- `.dk-harness/designs/metadata.json` — Figma file/node IDs
