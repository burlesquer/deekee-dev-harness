...
### Added
- **dk-harness-opus-audit.ts**: 4-way Decision Map — frontmatter/AGENT_TIERS/team-orchestrator/CLAUDE.md 교차 분석으로 21개 에이전트 모델 분류 (JUSTIFIED/DIVERGENT/ORPHAN_OPUS/DEAD_REF)
- **opus-baseline.ts**: runtime cost proxy ranking (duration_ms × tier pricing 기반 상대 비용 추정)
- **테스트 35개**: dk-harness-opus-audit(19) + opus-baseline(16)
...
- **/dk-harness cancel 스킬**: 활성 모드 graceful 종료 + transient 상태 정리, --force 지원
...
- **가드레일 출처 태그**: 차단 메시지에 `[dk-harness guardrail]` 접두사 추가
...
- **스킬 호출 형식 통일**: 스킬 내부 문서의 `/dk-harness team` (space) → `/dk-harness:team` (colon) 형식으로 전수 교체 — Claude Code 라우팅 오류 방지
- **OMC 플러그인 스킬 충돌 해소**: omc의 `team`, `tdd` 스킬을 비활성화하여 dk-harness 스킬과의 이름 충돌 제거

### Added

- **`/dk-harness:spring-boot` 스킬** (신규): Spring Boot + DDD 아키텍처 가이드
- **`/dk-harness:clean-code` 스킬** (신규): Robert C. Martin Clean Code 원칙 기반 코드 품질 감사 + 자동 수정
...
- **Task Checklist 미체크**: subagent-stop 훅에 auto-checkSubtask 로직 추가 — 에이전트 완료 시 `.dk-harness/tasks/`에서 에이전트명 매칭하여 subtask 자동 done 처리
...
- **`.dk-harness/state/tech-stack.json` 캐싱**: git commit 날짜 기반 invalidation — dep 파일이 변경되면 자동 재감지
...
- **worker prompt LIVE MONITOR 지시**: 에이전트가 파일 변경 시 `.dk-harness/state/agent-live.jsonl`에 이벤트 기록
...
- **preamble Directory Boundary**: T1~T4 전 tier에 `.dk-harness/` 디렉토리만 사용 규칙 추가
- **task from-plan 액션**: 플랜 Steps를 체크리스트로 자동 변환
- **plan-only → task 연결**: Step 4에서 `/dk-harness:task` 옵션 제공
...
- **`/dk-harness task` → `/dk-harness:task`**: 전체 스킬 콜론 포맷 통일
...
Claude Code 소스 분석(claude-code-analysis.md)을 기반으로 dk-harness이 Claude의 내장 기능을 억제하거나 중복하는 10개 항목을 식별하고 7건 수정.
...
- **Denial Audit Trail**: 가드레일 거부 이력을 `.dk-harness/logs/denials.jsonl`에 구조화 기록, 세션 종료 시 요약 출력 (`denial-tracker.ts`)
...
외부 스킬 3종(codebase-analysis, project-analyzer, lsp-code-analysis)을 분석하여 dk-harness 에이전트 체계에 맞게 커스텀한 프로젝트 탐색 스킬 추가.

### Added

- **dk-harness-scan**: project-analyzer 기반 Quick Project Profiling — 프로젝트 타입/프레임워크/구조/기술부채 자동 탐지 (Klay + Kain)
- **dk-harness-deep-explore**: codebase-analysis 기반 6-Phase 증거 기반 심층 분석 — file:line 증거 수집 (Klay + Kain + Sam)
- **dk-harness-lsp-nav**: lsp-code-analysis 기반 시맨틱 코드 네비게이션 — 정의/참조/흐름/계층/의존성 추적 (Kain)
...
토큰 사용량이 적은 사용자도 경량 프로필로 dk-harness 활용 가능.
...
- **User Agent Profile** (`scripts/routing/profile-resolver.ts`): 사용자별 에이전트 범위 + 비용 한도 설정
  ...
  - CLI 프리셋: `dk-harness config profile light|standard|full`
- **Config 3단 Merge** (`scripts/core/config.ts`): `.dk-harness/config.json` > `dk-harness.config.json` > DEFAULTS
...
- **token-tracker** (`scripts/telemetry/token-tracker.ts`): 에이전트별 + 단계별 토큰 추적
  - `logTokenUsage()`: `.dk-harness/telemetry/token-usage.jsonl`에 JSONL append
...
DKH-DR 합의 계획(Ryan→Able→Klay→Noa→Critic)으로 설계, Hybrid Parallel 3-iteration으로 실행.
6개 Phase 완료, 187 신규 테스트 추가 (전체 1,285 tests).

### Added

- **Project Rules Engine** (`scripts/rules/`): `.dk-harness/rules/*.json` 프로젝트별 커스텀 리뷰 규칙
...
`/dk-harness do` 직접 라우팅 7개를 프롬프트 의존에서 코드 강제로 전환.
...
## [2.8.9] - 2026-04-01 — DKH-DR Consensus Planning Framework

plan-task를 DKH-DR(6자 합의 계획 시스템)으로 전면 재설계.
...
- **State Management**: `.dk-harness/state/plan-state.json` — phase 추적, lifecycle, 세션 복원, 5 Mandatory Rules, Critic의 State Integrity Check
...
- **plan-task SKILL.md** 전면 재설계 (111줄 → 453줄)
  ...
  - RALPLAN-DR → DKH-DR (Constraints/Preferences/Drivers/Options)
  - 7-Phase Pipeline + Consensus Loop (low 3회, mid-high 5회)
  - 복잡도별 DR 분화: Lite(low) / Standard(mid) / Deep(high+deliberate)
- **able.md**: sonnet → **opus** 업그레이드, DKH-DR 출력 구조, Drivers 가변화, Rejection Evidence, Living ADR 생성
...
- **프로젝트 독립화:** 모든 외부 프레임워크 참조 제거, dk-harness 독자 브랜딩 확립
- docs/WHY-DK-HARNESS.md, ROADMAP-v3.md 전면 리라이트 (독자 가치 제안)
...
**`/dk-harness harness` — 하네스 설계 메타 스킬 (40번째 스킬)**
- ZIVO/harness 프로젝트를 분석·흡수하여 dk-harness 네이티브 스킬로 재구성
...
| 서브커맨드 | 엔진 | 기능 |
|-----------|------|------|
| `/dk-harness harness "task"` | `adaptive-architect.ts` (413 LOC) | 복잡도 기반 자동 설계 — 6개 도메인 프로파일, 패턴/에이전트/모델 자동 추천 |
| `/dk-harness harness check` | `validate-harness.ts` (272 LOC) | 구조/연결/품질/팀크기 4축 자동 검증 — 순환의존성·고립에이전트·dead link 탐지 |
| `/dk-harness harness find` | `harness-gallery.ts` (251 LOC) | 패턴 DB + 추천 — 5개 빌트인 패턴, 사용자 패턴 자동 등록, 성공률 기반 정렬 |
| `/dk-harness harness sim` | `harness-simulator.ts` (271 LOC) | 데이터 흐름 시뮬레이션 — Phase별 입출력 추적, 파일명 불일치·에러 시나리오 탐지 |
| `/dk-harness harness log` | `harness-evolution.ts` (236 LOC) | 버전 관리 + A/B 비교 — 스냅샷, 메트릭 기록, 개선/퇴보 판정 |
| `/dk-harness harness chain` | `harness-composer.ts` (204 LOC) | 멀티 하네스 파이프라인 — `"a → b → c"` 조합, 실행 계획 생성 |
| `/dk-harness harness fix` | `harness-debugger.ts` (292 LOC) | 런타임 진단 — 멈춘 에이전트·누락 산출물·단절 데이터 흐름 탐지 + 수정 제안 |
...
- `/dk-harness teacher <task>`: Socratic teaching mode that breaks down tasks into concepts.
...
- Full chain: /dk-harness ship (7 steps) → /dk-harness land-and-deploy (4 steps) = 11 steps
...
### vs OMC 비교 (코드 기반 분석)
- **dk-harness 8.05/10 vs OMC 8.78/10** (갭 1.68 → 0.73, **56% 축소**)
- Orchestration 영역 **역전** (dk-harness 8.5 > OMC 8.4)
...
- CLI tools: dk-harness-learn (JSONL learning management), dk-harness-bench (benchmark runner)
...
- dk-harness-config.mjs: dot-notation 설정 관리
- dk-harness-diff-scope.mjs: 6-category 변경 범위 감지
- dk-harness-analytics.mjs: 사용 통계 대시보드
- dk-harness-doctor.mjs: 설치 상태 진단
...
- `dk-harness-auto` 라벨이 붙은 이슈를 Claude Code CLI로 자동 분석/수정
...
- `.gitignore` 정리 — `.dk-harness/` 전체를 blanket ignore (런타임 파일 69개 git 트래킹 제거)
...
### Fixed — persist.mjs `--dir` 누락으로 .dk-harness/tasks 미생성 버그

- `persist.mjs` 호출 시 `--dir "$(pwd)"` 미전달로 cwd가 플러그인 디렉토리일 때 `.dk-harness/tasks/`가 사용자 프로젝트가 아닌 엉뚱한 위치에 생성되는 버그 수정
...
#### 신규 스킬 2개
- `/dk-harness flutter-animation`: Flutter 애니메이션 구현 가이드 (Implicit/Explicit/Hero/Physics-Based)
- `/dk-harness flutter-architecture`: Flutter 앱 아키텍처 설계 (레이어드 구조, 상태 관리, 프로젝트 구조화)
...
#### 리브랜딩 잔여 정리
- `.gitignore`: `.dk-harness/` 경로 → `.dk-harness/`으로 통일
- `banner.svg`: `sw` → `ai`, `dk-harness` → `dk-harness`, `HARNESS ENGINEERING AGENT` → `AI-NG ENGINEERING AGENT`
...
#### 신규 스킬 4개
- `/dk-harness refactor`: 구조적 리팩토링 — Klay(영향 분석) → Jay/Derek(실행) → Milla(검증), 자동 체크포인트
- `/dk-harness test`: 테스트 실행/커버리지 분석/누락 테스트 생성 — run/cover/gen 3모드
- `/dk-harness perf`: 성능 프로파일링 — Jun(분석) + Klay(구조), bundle/runtime/query 3모드
- `/dk-harness lsp`: 죽은 코드 탐지 — Simon(LSP/AST/Grep 3단계), `--fix`로 자동 제거
...
### Removed
- `commands/` 디렉토리 전체 삭제 (legacy delegation 래퍼 14개) — `/dk-harness:*` prefix 스킬만 사용
...
- `.dk-harness/state/version-check.json`에 캐시 저장

#### 랜딩 페이지 최신화
- 버전 2.2.3 → 2.3.3, GitHub URL `deekee-dev-harness`로 수정
- 삭제된 `/dk-harness cost` 커맨드 제거
- 신규 스킬 반영: `figma-read`, `progress-check`, `design`, `qa-loop`, `plan-task`, `review-code`
- Modes에 `figma→code`, `qa-loop` 추가

## [2.3.2] - 2026-03-26

### Changed — 프로젝트 리브랜딩: dk-harness → dk-harness (dk-harness)

#### 전체 리네이밍 (129개 파일)
- **프로젝트명**: `dk-harness` → `dk-harness`
- **패키지명**: `dk-harness-claude` → `dk-harness`
- **스킬 prefix**: `dk-harness:` → `dk-harness:` (모든 에이전트 호출)
- **슬래시 커맨드**: `/dk-harness` → `/dk-harness`
- **작업 디렉토리**: `.dk-harness/` → `.dk-harness/`
- **로깅 prefix**: `[dk-harness:*]` → `[dk-harness:*]`
- **Git remote**: `burlesquer/dk-harness-claude` → `burlesquer/deekee-dev-harness`
- **커맨드 파일**: `commands/dk-harness.md` → `commands/dk-harness.md`
...
#### 신규 스킬 2개
- `/dk-harness figma-read <url>`: Figma → 기획 문서 추출 (5섹션: 화면/플로우/컴포넌트/인터랙션/구현매핑힌트)
- `/dk-harness progress-check`: 기획 vs 코드 진행도 비교 (Figma 없이 수동 기획서도 지원)
...
#### 신규 스킬 4개
- **`/dk-harness do`**: 자연어 자동 라우팅 — intent-router.mjs가 입력 분석 → plan/auto/team/wizard 자동 선택
- **`/dk-harness debug`**: 과학적 디버깅 — 증상→가설→테스트→결론, `.dk-harness/debug/{slug}.md` 영구 상태
- **`/dk-harness init`**: 프로젝트 초기화 — 질문 기반 문맥 수집 → PROJECT.md + REQUIREMENTS.md + TECH-STACK.md
- **`/dk-harness cost`**: 비용/토큰 투명성 — 에이전트별 활동 기반 비용 추정 보고
...
- **status-view.mjs**: `.dk-harness/STATUS.md` 자동 생성 (인간 가독성 상태 뷰)
...
#### DX: 팀 배치 가시성 강화
- **user-prompt-submit.mjs**: 구분선(`━━━ dk-harness Team Deployment ━━━`) + Pipeline 요약 라인 추가

#### agent-ui SKILL.md 리팩터
- 기본 모드: `open (removed)` — 항상 클라우드 3D 오피스 브라우저 오픈
...
- **display.mjs**: `C.red` 컬러 코드 누락 → `/dk-harness help` 런타임 크래시 수정
...
#### New Commands (/ 자동완성)
- 14개 command 등록: auto, design, explore, help, plan, review, rollback, start, dk-harness, task, tdd, team, verify, wizard
...
agent-ui (removed) 연동 스킬 추가.

#### New Skill
- **agent-ui** — 3D 에이전트 오피스 시각화. `/dk-harness agent-ui`로 현재 세션을 브라우저에서 실시간 확인. `--setup`으로 Claude Code hooks 자동 설정, `--status`로 상태 확인, `--uninstall`로 제거.

#### Integration
- agent-ui (removed) ((removed)) 연동
...
Google Labs stitch-skills를 dk-harness에 통합. 7개 원본 skill을 5개로 재구성.
...
- 디자인 산출물: `.dk-harness/designs/`, 영상 산출물: `.dk-harness/videos/`
...
- **`logger.mjs`** — `process.cwd()` 하드코딩 → `DK_PROJECT_DIR` 환경변수 지원
...
- **Agent Visibility (omc-style)** — 모든 Agent 스폰 시 `description` 파라미터 필수
  - 터미널에 `⏺ dk-harness:jay(Jay: Backend API 구현) Sonnet` 자동 표시
...
### Added
- **`/dk-harness team [agents] "task"`** — staged pipeline with verify→fix loop
...
### Enhanced
- **`/dk-harness auto`** — `@{Name}❯` worker visibility
...
- **`/dk-harness plan`** — next-action flow after planning
  - Option 1: `/dk-harness team` (recommended, verify loop)
  - Option 2: `/dk-harness auto` (one-shot, fast)
  - Option 3: save only

## [1.8.2] - 2026-03-19

### Added
- **`/dk-harness start` Interactive Setup Wizard** — omc 벤치마킹 4-Phase 셋업
...
- **`scripts/setup/setup-progress.mjs`** — 셋업 진행 상태 관리
  ...
  - `~/.claude/.dk-harness-config.json`에 영구 설정 완료 마커 저장
  - `.dk-harness/state/setup-state.json`에 임시 진행 상태 (24h 자동 만료)
- **First Run Detection** — session-start에서 셋업 미완료 자동 감지
  - 미설정: "`/dk-harness start`를 실행하세요" 안내
...
- HUD 인라인 프롬프트 제거 → `/dk-harness start` 위자드에서 통합 처리
...
- **Status Line HUD** — 터미널 하단에 활성 에이전트를 깜빡이는 컬러 점으로 실시간 표시
  ...
  - `~/.claude/hud/dk-harness-hud.mjs`: 래퍼 스크립트 (dev/cache/marketplace 경로 자동 탐색)
...
- Session-start command list updated to `/dk-harness` unified commands (auto, tdd, task, wizard, help)
...
- **Auto Runner** — `/dk-harness auto` 전체 파이프라인 실행 계획 생성
...
- **Handoff Manager** — 단계 전환 시 .dk-harness/handoffs/ 자동 생성
...
- **Task Manager** — Main Task → Sub Tasks 계층 체크리스트
  ...
  - ☐/☑ 체크리스트 추적, 자동 완료 판정
- **Plan Manager** — .dk-harness/plans/ 문서 자동 생성
  - Plan 문서 + Task 체크리스트 동시 생성
- **9개 스킬 자동완성** — /dk-harness: 입력 시 11개 스킬 표시
...
### Added
- **Progress Tracker** — dk-harness-progress.md 생성, 세션 간 연속성
...
### Added
- `/dk-harness help` — 컬러풀한 에이전트 팀 시각화
