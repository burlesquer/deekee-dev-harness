/**
 * dk-harness Plan State Manager — Tracks DKH-DR consensus planning lifecycle.
...
function statePath(projectDir: string): string {
  return join(projectDir, '.dk-harness', 'state', 'plan-state.json');
}
 ...
 * Called by pre-tool-use hook on every dk-harness: agent spawn.
...
export function checkAgentAllowed(
  ...
  // Normalize agent name: "dk-harness:ryan" → "ryan", "Ryan" → "ryan"
  const normalized = agentName.replace(/^dk-harness:/, '').toLowerCase();
 ...
 * Called by post-tool-use hook when an dk-harness: agent finishes.
 *
...
export function autoAdvancePhase(
  ...
  const phase = state.phase;
  const allowedAgents = PHASE_AGENT_MAP[phase];
  const normalized = completedAgent.replace(/^dk-harness:/, '').toLowerCase();
