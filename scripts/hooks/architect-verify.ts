/**
 * dk-harness Architect Verification — Enforces architect approval after team-verify.
...
function verifyPath(projectDir: string): string {
  return join(projectDir, '.dk-harness', 'state', 'architect-verify.json');
}
...
export function generateArchitectPrompt(state: ArchitectVerifyState): string {
  const lines: string[] = [
    `[dk-harness:architect-verify] VERIFICATION PENDING (attempt ${state.verificationAttempts + 1}/${state.maxVerificationAttempts})`,
    `Feature: ${state.feature}`,
