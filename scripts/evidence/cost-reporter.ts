...
export function generateCostReport(projectDir?: string): CostReport {
  ...
  const costPath  = join(dir, '.dk-harness', 'state', 'cost-tracker.json');
  const tracePath = join(dir, '.dk-harness', 'state', 'agent-traces.json');
