/**
 * dk-harness Evidence Report Generator
...
export function generateReport(feature: string, options: ReportOptions = {}, projectDir?: string): ReportResult {
  ...
  const pdcaState = readState(join(dir, '.dk-harness', 'state', 'pdca-status.json')) as { ok: boolean; data?: PdcaState };
...
`;
  ...
  const reportPath = join(dir, '.dk-harness', 'reports', `${date}-${feature}.md`);

  try {
    mkdirSync(join(dir, '.dk-harness', 'reports'), { recursive: true });
