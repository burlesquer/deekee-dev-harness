/**
 * dk-harness Status View — Human-Readable PDCA Status Generator
 * Reads pdca-status.json and writes .dk-harness/STATUS.md.
 *
...
export function generateStatusView(projectDir: string): StatusViewResult {
  const statePath = join(projectDir, '.dk-harness', 'state', 'pdca-status.json');
  const outputPath = join(projectDir, '.dk-harness', 'STATUS.md');
  ...
  if (zombieCount > 0) {
    lines.push(`## Warnings`);
    lines.push(`- ${zombieCount} zombie feature(s) detected (no activity > ${ZOMBIE_AGE_DAYS} days). Run \`/dk-harness gc\` to clean up.`);
    lines.push('');
  }
