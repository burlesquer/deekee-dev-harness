/**
 * dk-harness State GC — Zombie Feature Garbage Collector
...
function getStatePath(projectDir: string): string {
  return join(projectDir, '.dk-harness', 'state', 'pdca-status.json');
}

function getArchiveDir(projectDir: string): string {
  return join(projectDir, '.dk-harness', 'archive');
}
...
export function runGC(projectDir: string, options: GCOptions = {}): GCResult {
  ...
  // Write archive file: .dk-harness/archive/gc-{date}-{ms}.json
