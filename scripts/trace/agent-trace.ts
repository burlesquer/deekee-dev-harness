/**
 * dk-harness Agent Trace Analyzer v0.4.0
...
function getTracePath(projectDir?: string): string {
  return join(projectDir || process.cwd(), '.dk-harness', 'state', 'agent-traces.json');
}
...
export function formatTraceSummary(projectDir?: string): string {
  ...
  if (totalEvents === 0) return '[dk-harness Trace] No traces recorded.';

  const lines: string[] = [
    `[dk-harness Trace] ${totalEvents} events recorded`,
    ''
  ];
