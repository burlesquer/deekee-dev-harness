/**
 * dk-harness Team Orchestrator v1.2.0
 ...
 * Instead of fixed N workers, dk-harness auto-selects based on task analysis.
 */
 ...
 * dk-harness innovation: OMC는 사용자가 N을 지정, dk-harness은 자동 분석.
 */
...
/**
 * Generate worker preamble — dk-harness harness-enhanced version.
...
export function formatTeamSelection(selection: TeamSelection): string {
  ...
  const lines: string[] = [
    `[dk-harness Team] Auto-selected: ${team.name}`,
    `  ${reason}`,
    '',
    '  ┌──────────────┬─────────────────────────┬─────────┬──────────────────────┐',
