/**
 * dk-harness Task Manager v1.1.0
 * Hierarchical task system: Main Task → Sub Tasks with checklist tracking.
 * Stored in .dk-harness/tasks/ as individual JSON files.
...
function getTaskDir(projectDir?: string): string {
  const dir = join(projectDir || process.cwd(), '.dk-harness', 'tasks');
...
export function formatTaskChecklist(taskId: string, projectDir?: string): string {
  ...
  if (!task) return `[dk-harness Task] Task "${taskId}" not found`;
...
export function formatAllTasks(projectDir?: string): string {
  ...
  if (tasks.length === 0) return '[dk-harness Tasks] No tasks registered';

  const lines: string[] = ['[dk-harness Tasks]', ''];
