/**
 * dk-harness Eval Store — Persistent eval result storage
 *
 * Stores eval runs as JSON files under .dk-harness/evals/ using the
...
function evalsDir(projectDir: string): string {
  return join(projectDir, '.dk-harness', 'evals');
}
