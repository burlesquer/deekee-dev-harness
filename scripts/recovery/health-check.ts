/**
 * dk-harness Health Check (Innovation #5 — Self-Healing Engine)
...
/**
 * Run health check on all dk-harness state files.
 */
export function runHealthCheck(projectDir?: string): HealthCheckResult {
  ...
  checks.push(checkJsonFile(join(dir, '.dk-harness', 'state', 'pdca-status.json'), 'pdca-status'));

  // Check project memory
  checks.push(checkJsonFile(join(dir, '.dk-harness', 'project-memory.json'), 'project-memory'));

  // Check routing history
  checks.push(checkJsonFile(join(dir, '.dk-harness', 'routing-history.json'), 'routing-history'));

  // Check circuit breaker state
  checks.push(checkJsonFile(join(dir, '.dk-harness', 'state', 'circuit-breaker.json'), 'circuit-breaker'));
