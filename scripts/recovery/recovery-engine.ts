/**
 * dk-harness Recovery Engine (Innovation #5 — Self-Healing Engine)
...
export function recoverState(stateFile: string, projectDir?: string): RecoveryResult {
  ...
  const emergencyPath = join(dir, '.dk-harness', 'state', `${stateFile.replace('.json', '')}-emergency-backup.json`);
  if (existsSync(emergencyPath)) {
    const result = readState(emergencyPath) as StateReadResult;
    if (result.ok && result.data?.state) {
      const restored = writeState(join(dir, '.dk-harness', 'state', stateFile), result.data.state) as StateWriteResult;
      if (restored.ok) {
  ...
  const snapshotDir = join(dir, '.dk-harness', 'snapshots');
  ...
  const restored = writeState(join(dir, '.dk-harness', 'state', stateFile), freshState) as StateWriteResult;
  if (restored.ok) {
