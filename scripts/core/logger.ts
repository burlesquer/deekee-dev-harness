/**
 * dk-harness Structured Logger
 * All output goes to stderr (never stdout — stdout is reserved for hook responses).
 * Logs also persist to .dk-harness/logs/ for debugging.
...
const MIN_LEVEL: number = LOG_LEVELS[(process.env.DK_LOG_LEVEL || 'info') as LogLevel] ?? LOG_LEVELS.info;
...
function log(level: LogLevel, module: string, message: string, data?: unknown): void {
  ...
  const line = `[dk-harness:${module}] ${level.toUpperCase()}: ${message}`;
  if (level === 'error') {
...
function persistLog(entry: LogEntry): void {
  try {
    const logDir = join(process.env.DK_PROJECT_DIR || process.cwd(), '.dk-harness', 'logs');
