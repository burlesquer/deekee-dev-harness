...
// -- Parse transcript tail for active dk-harness agents --
      ...
      try {
        ...
        if (entry.type === 'tool_use') {
          ...
          if ((toolName === 'Agent' || toolName === 'Task') &&
              ...
              input.subagent_type.startsWith('dk-harness:')) {
            const agentKey = input.subagent_type.replace('dk-harness:', '');
...
function getUpdateInfo(cwd: string): string | null {
  const cacheDir = join(cwd, '.dk-harness', 'state');
  ...
  try {
    ...
    const script = `
      ...
      const fs = require('fs');
      const url = 'https://raw.githubusercontent.com/burlesquer/deekee-dev-harness/main/package.json';
 ...
 * Reads .dk-harness/state/team-health.json synchronously (HUD runs sync).
 * Returns null if no team health data exists.
 */
export function getTeamStatusLine(cwd: string): string | null {
  try {
    const healthPath = join(cwd, '.dk-harness', 'state', 'team-health.json');
...
function getPdcaStage(cwd: string): PdcaInfo | null {
  try {
    const statePath = join(cwd, '.dk-harness', 'state', 'pdca-status.json');
...
function main(): void {
  ...
  if (!stdin) {
    console.log(`${BOLD}dk-harness${RESET}`);
    return;
  }
  ...
  // dk-harness brand
  parts.push(`${BOLD}dk-harness${RESET}`);
