#!/usr/bin/env node
/**
 * dk-harness agent-ui: Open 3D Office with session context
 *
...
function getActiveAgents(projectDir: string): AgentInfo[] {
  const tracePath = join(projectDir, '.dk-harness', 'state', 'agent-traces.json');
...
function getPipelineState(projectDir: string): PipelineInfo | null {
  const statePath = join(projectDir, '.dk-harness', 'state', 'pipeline-state.json');
...
params.set('user', process.env.DK_AGENT_NAME || process.env.USER || 'unknown');
params.set('team', process.env.DK_TEAM_ID || 'default');
