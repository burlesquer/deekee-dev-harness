#!/usr/bin/env node
/**
 * dk-harness Persistence CLI
 * Called by skills to persist plans and tasks to .dk-harness/ directories.
 *
 * Usage:
  ...
  switch (command) {
    case 'plan': {
      if (useStdin) {
        ...
        // Quality Gate check (if DKH-DR fields present)
        if (data.noaVerdict || data.criticVerdict) {
          ...
          if (!qg.pass) {
            console.error(`[dk-harness:quality-gate] FAIL — ${qg.failures.join('; ')}`);
