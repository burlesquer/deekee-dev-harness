/**
 * dk-harness Careful Guard (Phase 4 — 200% Differentiator)
 *
 ...
 * Check a file path against freeze boundaries using dk-harness's config system.
 */
...
export function formatCarefulResult(result: CarefulResult): string {
  ...
  return `[dk-harness Careful Guard] ${icon} ${result.message}`;
}
