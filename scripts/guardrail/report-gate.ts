/**
 * dk-harness Report Gate (HOOK-4 — Hard Limit 5)
 *
...
/**
 * Count completed tasks in the dk-harness task index.
 * Returns 0 if the index cannot be read.
 */
function countCompletedTasks(projectDir: string): number {
  try {
    const indexPath = join(projectDir, '.dk-harness', 'tasks', '_index.json');
 ...
 * - If a valid report is found in .dk-harness/reports/, ok: true.
...
export function checkCompletionReport(projectDir?: string): ReportCheckResult {
  ...
  const reportsDir = join(dir, '.dk-harness', 'reports');
  ...
  try {
    ...
    return {
      ok: false,
      reason: '완료 보고서가 없습니다. .dk-harness/reports/ 디렉토리에 보고서를 작성하세요.',
    };
  }
}
