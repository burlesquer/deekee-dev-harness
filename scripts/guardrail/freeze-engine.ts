/**
 * dk-harness Freeze Engine — Directory-Scoped Edit Restriction
 *
 * Supports two modes:
...
export function checkFreeze(filePath: string, projectDir?: string): FreezeCheckResult {
  ...
  return {
    allowed: false,
    reason: mode === 'single'
      ? `File ${absolutePath} is outside freeze boundary ${pathList}. Run /dk-harness unfreeze to remove restriction.`
      : `File ${absolutePath} is outside allowed paths [${pathList}]. Run /dk-harness unfreeze to remove restriction.`,
  };
}
