/**
 * dk-harness Dry-Run Mode v0.3.0
...
function getDryRunPath(projectDir?: string): string {
  return join(projectDir || process.cwd(), '.dk-harness', 'state', 'dry-run-queue.json');
}
...
export function formatPreview(projectDir?: string): string {
  ...
  if (pending.length === 0) return '[dk-harness Dry-Run] 대기 중인 변경사항이 없습니다.';

  const lines: string[] = [
    '[dk-harness Dry-Run] 다음 변경사항이 대기 중입니다:',
    ''
  ];
  ...
  lines.push('');
  lines.push('진행하려면 "/dk-harness approve", 취소하려면 "/dk-harness reject"를 실행하세요.');
