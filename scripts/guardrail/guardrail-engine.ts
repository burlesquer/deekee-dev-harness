/**
 * dk-harness Guardrail Engine v0.3.0
...
const DEFAULT_RULES: GuardrailRule[] = [
  ...
  // dk-harness adds warn-level context only, to avoid double-blocking with Claude's built-in checks.
  {
    id: 'warn-rm-rf',
    type: 'bash',
  ...
  // Note: bash-type rules remain 'warn' to avoid double-blocking with Claude's built-in AST checks.
  {
    ...
    action: 'block',
    severity: 'high',
    message: '[dk-harness guardrail] .env 파일 수정이 차단되었습니다. 민감 정보가 포함될 수 있습니다.'
  },
  {
    id: 'protect-lockfile',
    type: 'file',
    ...
    action: 'block',
    severity: 'medium',
    message: '[dk-harness guardrail] 잠금 파일 직접 수정이 차단되었습니다. 패키지 매니저를 통해 수정하세요.'
  },
  {
    id: 'protect-ci',
    type: 'file',
    ...
    action: 'block',
    severity: 'high',
    message: '[dk-harness guardrail] CI/CD 설정 수정이 차단되었습니다. 변경사항을 신중히 검토하세요.'
  }
];
...
export function formatViolations(violations: Violation[]): string {
  ...
  const lines: string[] = ['[dk-harness Guardrail]'];
