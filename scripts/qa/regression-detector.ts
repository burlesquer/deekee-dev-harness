/**
 * dk-harness Regression Detector — Compare test results against baseline
...
export function saveBaseline(feature: string, results: TestResults, projectDir?: string): void {
  ...
  const path = join(dir, '.dk-harness', 'qa', `baseline-${feature}.json`);
  writeState(path, {
    feature,
    ...results,
...
export function formatRegression(result: RegressionResult): string {
  if (result.details?.noBaseline) return 'No baseline found. Run /dk-harness qa-loop first to establish baseline.';
