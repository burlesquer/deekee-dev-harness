...
export async function buildSnapshot(
  ...
  if (options.annotate) {
    const screenshotPath = options.outputPath ?? '/tmp/dk-harness-browse-annotated.png';
...
async function captureAnnotatedScreenshot(
  ...
  await page.evaluate((boxData) => {
    ...
    overlay.id = '__dk_annotation_overlay';
  ...
  // Remove overlay
  await page.evaluate(() => {
    document.getElementById('__dk_annotation_overlay')?.remove();
  });
}
