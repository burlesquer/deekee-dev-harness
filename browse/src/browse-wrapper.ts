/**
 * Browse Wrapper — Programmatic API over the dk-harness-browse daemon.
 *
...
export class BrowseWrapper {
  ...
  constructor(projectRoot: string, options?: { timeout?: number }) {
    const stateDir = join(projectRoot, '.dk-harness');
  ...
  async connect(): Promise<Result<DaemonState>> {
    ...
    if (!state) {
      return { ok: false, error: 'Browse daemon not running. Start with: dk-harness-browse status' };
    }
