export async function handleMetaCommand(cmd, args, bm, shutdown) {
    ...
    try {
        switch (cmd) {
            case 'screenshot': {
                let selector;
                let outputPath = `/tmp/dk-harness-browse-${Date.now()}.png`;
                let fullPage = false;
                let clip;
                const remaining = [];
            ...
            case 'pdf': {
                const outputPath = args[0] || `/tmp/dk-harness-browse-${Date.now()}.pdf`;
                await page.pdf({ path: outputPath, format: 'A4', printBackground: true });
                return ok(`PDF saved: ${outputPath}`);
            }
            case 'responsive': {
                const prefix = args[0] || `/tmp/dk-harness-responsive-${Date.now()}`;
                const viewports = [
