/**
 * dk-harness UserPromptSubmit Hook v1.6.0
...
const KEYWORD_ROUTES: KeywordRoute[] = [
  {
    ...
    skill: 'debug',
    message: '[SKILL SUGGESTION: /dk-harness debug] — Use the scientific debugging workflow.',
  },
  {
    keywords: ['qa', 'test loop', '테스트루프', '테스트 반복'],
    skill: 'qa-loop',
    message: '[SKILL SUGGESTION: /dk-harness qa] — Start QA cycle (test->fix->repeat).',
  },
  {
    keywords: ['plan', '계획', '기획', '설계'],
    skill: 'plan-task',
    message: '[SKILL SUGGESTION: /dk-harness plan] — Klay(Architect) will create a plan.',
  },
  {
    keywords: ['review', '리뷰', '코드리뷰', 'code review'],
    skill: 'review-code',
    message: '[SKILL SUGGESTION: /dk-harness review] — Milla(Security) + Sam(CTO) review.',
  },
  {
    keywords: ['rollback', '롤백', 'undo', '되돌리기', 'revert'],
    skill: 'rollback',
    message: '[SKILL SUGGESTION: /dk-harness rollback] — Rollback to last checkpoint.',
  },
];
...
/**
 * Find the most recent plan file in .dk-harness/plans/.
 * Returns filename or null.
 */
function findLatestPlan(projectDir: string): string | null {
  const plansDir = join(projectDir, '.dk-harness', 'plans');
...
try {
  ...
  if (isPlanRequest) {
    ...
    if (gateResult.verdict === 'BLOCK') {
      parts.push(`[dk-harness:plan-gate] BLOCK — ${gateResult.reason}`);
  ...
  if (planResume.canResume) {
    parts.push(`[dk-harness:plan-state] 이전 계획 세션 감지: "${planResume.feature}" — Phase: ${planResume.phase}, Iteration: ${planResume.iteration}`);
    parts.push(`재개하려면 /dk-harness plan 실행. 새로 시작하려면 .dk-harness/state/plan-state.json 삭제.`);
  }
    ...
    try {
      ...
      if (codexInfo.tier !== 'none') {
        ...
        const codexCmd = codexInfo.tier === 'plugin'
          ...
          : 'Codex CLI';
        parts.push(`[dk-harness:codex] Codex가 감지되었습니다. ${taskLabel}를 Codex와 협업하시겠습니까? (${codexCmd} 사용 가능)`);
      }
  ...
  if (session.active) {
    ...
    if (wantsExecution) {
      ...
      if (planFile) {
        parts.push(`[PLAN EXISTS: .dk-harness/plans/${planFile} — execute with /dk-harness team or /dk-harness auto]`);
      }
    }
  }
  ...
  const hasPlanSession: boolean = (() => {
    try {
      const planPath = join(projectDir, '.dk-harness', 'state', 'plan-state.json');
      return existsSync(planPath);
    } catch { return false; }
  })();
  if (isExecutionIntent && !hasPlanSession && !lower.includes('force:')) {
    parts.push(`[dk-harness:hard-limit] 팀 사이즈 미분석. 구현 전 /dk-harness plan 또는 /dk-harness auto로 팀 구성을 먼저 결정하세요. (Solo/Duo/Squad/Full)`);
  }
  ...
  parts.push(`━━━ dk-harness Team Deployment ━━━`);
  ...
  if (intent.isWizardMode) {
    ...
    parts.push('Klay(Architect) explores code -> Able(PM) creates plan -> saved to .dk-harness/plans/ -> run "/dk-harness auto" to execute.');
  ...
  process.stderr.write(`[dk-harness:user-prompt] ${(err as Error).message}\n`);
