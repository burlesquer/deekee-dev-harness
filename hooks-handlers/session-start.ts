/**
 * dk-harness SessionStart Hook v2.0.0
 * Injects harness rules so dk-harness is ALWAYS active without explicit invocation.
 * Detects first-run and prompts for setup via /dk-harness start.
 */
...
try {
  ...
  const config: DkHarnessConfig = loadConfig(projectDir);
  ...
  const pdcaState = readStateOrDefault(join(projectDir, '.dk-harness', 'state', 'pdca-status.json'), null) as PdcaState | null;
  const memory = readStateOrDefault(join(projectDir, '.dk-harness', 'project-memory.json'), null) as ProjectMemory | null;

  const ctx: string[] = [];

  // === Header ===
  ctx.push(`# dk-harness v2.4.8 Harness Engineering Agent`);
  ...
  if (!setupStatus.completed) {
    ...
    ctx.push(`dk-harness has not been initialized yet.`);
    ctx.push(`Run \`/dk-harness start\` to launch the setup wizard.`);
    ctx.push('');
    ...
    ctx.push('');
    ctx.push(`dk-harness features work without setup, but setup provides a better experience.`);
    ctx.push('');
  } else {
  ...
  if (setupStatus.completed && pdcaState?.activeFeature) {
    ...
    if (feat) {
      ...
      ctx.push(`To continue previous work, run \`/dk-harness next\`.`);
      ctx.push(`To start new work, run \`/dk-harness auto <feature> <task>\`.`);
      ctx.push('');
    }
  ...
  // === Agent Team ===
  ctx.push(`## dk-harness Agent Team`);
  ...
  const injectionMode: string = process.env.DK_FULL_INJECT === '1' ? 'full' : (config.context?.injectionMode || 'compact');

  if (injectionMode === 'compact') {
    ctx.push(`## dk-harness Mandatory Rules (compact)`);
    ctx.push('');
    ...
    ctx.push(`## dk-harness Mandatory Rules`);
    ctx.push('');
    ...
    ctx.push('```');
    ctx.push(`dk-harness:klay(Klay: architecture exploration + structural analysis) Opus`);
  ...
  // === Status Line Setup (now handled by /dk-harness start wizard) ===
  if (!setupStatus.completed) {
    const hudSetupFlag: string = join(projectDir, '.dk-harness', 'state', 'hud-setup-done');

    if (!existsSync(hudSetupFlag)) {
      try {
        const stateDir: string = join(projectDir, '.dk-harness', 'state');
  ...
  // === Commands ===
  ctx.push(`## Commands`);
  ctx.push(`/dk-harness start|status|next|reset|auto|tdd|task|explore|plan|execute|review|verify|wizard|rollback|learn|help`);
  ...
  try {
    ...
    if (ex(claudeMdPath)) {
      ...
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          additionalContext: `[dk-harness fallback] 동적 세션 주입에 실패했습니다. 정적 CLAUDE.md 규칙을 따르세요.\n\n${staticRules}`
        }
      }));
      wroteOutput = true;
    }
