/**
 * dk-harness SubagentStart Hook Handler v2.0.0
 * Tracks agent spawn events: name, subagent_type, model, timestamp.
 * Appends to .dk-harness/state/agent-trace.json and reports active agent count.
 *
 * Claude Code SubagentStart hook input:
 *   { agent_id, agent_type, session_id, cwd, hook_event_name }
 *   - agent_type = subagent_type passed to Agent() (e.g. "dk-harness:jay")
...
function getAgentTracePath(projectDir: string): string {
  return join(projectDir, '.dk-harness', 'state', 'agent-trace.json');
}
...
try {
  ...
  // Claude Code provides agent_type at top level (e.g. "dk-harness:jay")
  ...
  if (!agentName && subagentType !== 'unknown') {
    agentName = subagentType.replace(/^dk-harness:/, '');
  }
  ...
  const context = [
    `[dk-harness:agent-trace] SubagentStart: ${agentName} (${subagentType}) model=${model}`,
    `Active agents: ${activeCount} | Total spawned this session: ${store.totalSpawned}`,
    `[dk-harness:context-rule] You are a SUB-AGENT. Keep work focused and efficient:`,
