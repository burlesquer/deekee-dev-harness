/**
 * dk-harness PreCompact Hook Handler v0.4.0
...
try {
  ...
  const snapshotDir: string = join(projectDir, '.dk-harness', 'snapshots');
  ...
  const compactionStatePath = join(projectDir, '.dk-harness', 'state', 'compaction-session.json');
  ...
  if (budgetRatio > WARN_THRESHOLD) {
    ...
    budgetLines.push(`[dk-harness:compaction] Context budget at ~${pct}% (~${budget.total} / ${maxTokens} tokens). Compaction recommended.`);
