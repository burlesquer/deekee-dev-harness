/**
 * dk-harness PostToolUse Hook v1.5.0
 */
...
function isPdcaActive(dir: string): boolean {
  try {
    const stateFile = join(dir, '.dk-harness', 'state', 'pdca-status.json');
...
function detectStage(dir: string): string {
  try {
    const sessionPath = join(dir, '.dk-harness', 'state', 'team-session.json');
...
try {
  ...
  if (toolName) {
    ...
    if ((toolName === 'Agent' || toolName === 'Task') && toolInput.subagent_type) {
      const agentKey: string = (toolInput.name as string) || (toolInput.subagent_type as string).replace('dk-harness:', '');
      const agentName: string = (toolInput.name as string) || (toolInput.subagent_type as string).replace('dk-harness:', '');
      ...
      // DKH-DR phase auto-advance: when an dk-harness: agent completes, advance to next phase
      ...
      if (subagentType.startsWith('dk-harness:')) {
        try {
          ...
          if (newPhase) {
            process.stderr.write(`[dk-harness:phase-gate] Phase advanced → ${newPhase}\n`);
          }
        } catch { /* phase advance is best-effort */ }
      }

      // Activity summary: extract file operations from agent response and show inline
      if (subagentType.startsWith('dk-harness:') && toolResponse) {
        try {
          const files: string[] = [];
          ...
          if (unique.length > 0 || bashCount > 0) {
            ...
            process.stderr.write(`[dk-harness:${agentName}] ${summary}${more}\n`);
          }
      ...
      try {
        if (!isDryRunActive(projectDir)) {
          ...
          for (const rc of rcResults) {
            if (rc.verdict === 'block') {
              process.stderr.write(`[dk-harness:reality-check] BLOCK — ${rc.scenario}: ${rc.evidence}\n`);
            } else if (rc.verdict === 'warn') {
              process.stderr.write(`[dk-harness:reality-check] WARN — ${rc.scenario}: ${rc.evidence}\n`);
            }
          }
        }
      } catch { /* reality check is best-effort */ }
    } else if (toolName === 'SendMessage' && toolInput.to) {
      const agentKey: string = (toolInput.to as string).replace('dk-harness:', '');
  ...
  if (toolName === 'Bash' && toolResponse) {
    ...
    if (ev) process.stderr.write(`[dk-harness:evidence] ${ev.type}: ${ev.result}\n`);
    ...
    if (isError) {
      ...
      try {
        if (!isPdcaActive(projectDir)) {
          ...
          const errState = readState(join(projectDir, '.dk-harness', 'state', 'error-recovery.json'));
  ...
  if ((toolName === 'Bash' || toolName === 'Glob') && toolResponse.length > 0) {
    ...
    try {
      ...
      if (cmd.length > 10) {
        ...
        if (pattern) {
          process.stderr.write(`[dk-harness:learn] ${pattern.type}: ${pattern.suggestion || pattern.pattern}\n`);
        }
      }
  ...
  process.stderr.write(`[dk-harness:post-tool-use] ${(err as Error).message}\n`);
  process.stdout.write('{}');
}
