/**
 * dk-harness StopFailure Hook v1.3.2
 */
...
try {
  ...
  const sf: string = join(dir, '.dk-harness', 'state', 'pdca-status.json');
  const sr = readState(sf);
  if (sr.ok) {
    writeState(join(dir, '.dk-harness', 'state', 'pdca-emergency-backup.json'), {
      backupAt: new Date().toISOString(), reason: cat, state: sr.data
    });
  }

  try {
    const ld: string = join(dir, '.dk-harness', 'logs');
  ...
  process.stdout.write(JSON.stringify({
    stopReason: `[dk-harness Self-Healing] ${cat} -- ${rec}`
  }));
} catch (err: unknown) {
  process.stderr.write(`[dk-harness:stop-failure] ${(err as Error).message}\n`);
