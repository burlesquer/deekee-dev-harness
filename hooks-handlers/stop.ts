/**
 * dk-harness Stop Hook Handler v1.1.0
...
try {
  ...
  const stateFile: string = join(projectDir, '.dk-harness', 'state', 'pdca-status.json');
  ...
  if (session.active) {
    ...
    const result: HandoffResult = writeStageHandoff({
      ...
      decisions: [`Mode: ${session.mode}`, `Resume with /dk-harness team or /dk-harness auto`],
  ...
  // Check for active DKH-DR planning session (blocks stop during consensus)
  ...
  if (planState?.active && planState.phase !== 'completed' && planState.phase !== 'terminated') {
    ...
    ctx.push(`[dk-harness:plan-gate] DKH-DR 합의 진행 중 — 중단할 수 없습니다.`);
    ctx.push(`Feature: "${planState.feature}" | Phase: ${phaseName} | Iteration: ${planState.iteration}/${planState.maxIterations}`);
    ctx.push(`합의 완료 후 자동 종료됩니다. 강제 종료: /dk-harness cancel`);
  ...
  if (persistentMode?.active) {
    ...
    if (!canBlock) {
      ...
      ctx.push(`[dk-harness:persistent-mode] Circuit breaker — 세션 종료를 허용합니다.`);
    } else {
      ...
      // --- Priority 2: PRD stories incomplete ---
        ...
        if (prdStatus.total > 0 && !prdStatus.allComplete) {
          ctx.push(`[dk-harness:persistent-mode] Mode: ${modeLabel} | Iteration ${iteration}/${maxIter}`);
          ctx.push(`[dk-harness:prd] 스토리 진행: ${prdStatus.completed}/${prdStatus.total} 완료`);
          if (prdStatus.nextStory) {
            ctx.push(`다음 스토리: ${prdStatus.nextStory.id} "${prdStatus.nextStory.title}"`);
          }
          ctx.push(`작업이 완료되지 않았습니다. 계속하세요.`);
          ctx.push(`완료 후 /dk-harness team --complete 또는 cancel로 종료하세요.`);
  ...
  try {
    ...
    if (!reportCheck.ok) {
      const templateMsg = reportCheck.templatePath
        ? `[dk-harness:hard-limit] 완료 보고서 누락. ${reportCheck.templatePath}에 템플릿을 생성했습니다.`
        : `[dk-harness:hard-limit] 완료 보고서 누락. .dk-harness/reports/에 보고서(Team/Agents/Completeness/Evidence/Verdict)를 작성하세요.`;
  ...
  try {
    ...
    if (!gate.ok) {
      ctx.push(`[dk-harness:evidence-warning] 세션 종료 시 미검증 태스크 감지. ${gate.reason}`);
    }
